...
SCHEDULE_PROGRAMS = [
    ...
    ('やってM!LK',
     'TV', 'TBS テレビ', '定期放送中',
     'https://www.tbs.co.jp/yatte_milk/', BADGE_COLORS['TV'], 'active'),
    ('レコメン!',
     'ラジオ', '文化放送 毎週月〜金', '定期放送中',
     'https://www.joqr.co.jp/qr/program/reco/', BADGE_COLORS['ラジオ'], 'active'),
    ('イマドキッ ドゥフドゥフナイト',
     'ラジオ', 'MBSラジオ', '定期放送中',
     'https://www.mbs1179.com/imadoki/', BADGE_COLORS['ラジオ'], 'active'),
    # ── 公開予定・近日 ──
    ('君の好きは無敵',
     'TV', 'TBS テレビ 火曜22:00〜', '放送予定',
     'https://www.tbs.co.jp/kiminosukihamuteki_tbs/', BADGE_COLORS['TV'], 'upcoming'),
    ('トイ・ストーリー５',
     '映画', 'Disney / 2026年夏公開予定', '公開予定',
     'https://www.disney.co.jp/movie/toy5', BADGE_COLORS['映画'], 'upcoming'),
    ('仮面ライダーゼッツ＆超宇宙刑事ギャバン',
     '映画', '2026年夏公開予定', '公開予定',
     'https://zeztz-gavan-26movie.com/', BADGE_COLORS['映画'], 'upcoming'),
    # ── リリース ──
    ('M!LK 1st Mini Album（タイトル未定）',
     'リリース', '2026年09月16日発売', '予約受付中',
     'https://victor-store.jp/', BADGE_COLORS['リリース'], 'release'),
]
...
KNOWN_GOODS = [
    {
        ...
        'price':    '¥2,530（税込）',
        'url':      'https://victor-store.jp/search/?keyword=M!LK',
        'image':    '',
        'date':     '2026-09-16',
    },
    {
...
def generate_schedule_html(articles, updated_at):
    ...
    # Show ALL of today's articles (no keyword filter)
    today_arts = sorted(
    ...
    # Build program cards — split active vs upcoming
    def _prog_card(name, badge, station, sched, url, color):
        return (
    ...
    upcoming_cards = []
    release_cards  = []
    for name, badge, station, sched, url, color, status in SCHEDULE_PROGRAMS:
        card = _prog_card(name, badge, station, sched, url, color)
        if status == 'active':
