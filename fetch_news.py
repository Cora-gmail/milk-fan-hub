...
def fetch_oricon():
    ...
    # ① 検索ページ（M!LK 専用クエリ、最優先）
    search_arts = _scrape_search('Oricon',
        'https://www.oricon.co.jp/search/?q=m%21lk&cat=news',
        'https://www.oricon.co.jp',
        ['.news-list li', '.list-news li', 'ul.list li',
         '.search-result li', 'article', '.item'],
        keyword_check=None,   # 検索結果は既に M!LK 絞り込み済み
        limit=15)
    ...
    print(f'  -> {len(arts)} articles (search:{len(search_arts)} rss:{len(rss_arts)})')
    return arts
