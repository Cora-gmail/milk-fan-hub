...
def fetch_oricon_daily_charts():
    ...
    daily_dates = [(today - timedelta(days=d)).strftime('%Y-%m-%d') for d in range(30)]
...
def fetch_sd_milk_release_news(ex_ids=None, max_per_cat=15):
    """Scrape sd-milk.com/contents/news/release and /goods, fetch each article for product details."""
    ...
    for base_url, cat_label in CAT_URLS:
        ...
        for page in range(1, 5):
            ...
            try:
                ...
                soup = BeautifulSoup(r.text, 'html.parser')
                article_urls = []
                # Regex on full raw HTML — finds /contents/\d+ anywhere (__NEXT_DATA__ JSON, href attrs, etc.)
                for m in re.finditer(r'/contents/(\d{4,})', r.text):
                    full_url = 'https://sd-milk.com/contents/' + m.group(1)
                ...
                for art_url in article_urls:
                    ...
                    if art_id in ex_ids:
                        found += 1
                        continue  # already stored, skip fetch
...
def main():
    ...
    # Repair pass: re-fetch body for articles missing body_ja (new selectors catch more sites)
    _body_repaired = 0
    _body_cutoff   = (datetime.now(JST) - timedelta(days=90)).strftime('%Y-%m-%d')
    for _a in existing:
        if _body_repaired >= 50:
            break
        if _a.get('body_ja') or not _a.get('url'):
            continue
    ...
    if added_daily:
        daily_cutoff = (datetime.now(JST).replace(tzinfo=None) - timedelta(days=45)).strftime('%Y-%m-%d')
