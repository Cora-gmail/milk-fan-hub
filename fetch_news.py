...
def translate(text):
    if not translator or not text or not text.strip():
        return ''
    # 用私有 Unicode 字元保留段落分隔，DeepL 不會翻譯它
    SEP = ''
    encoded = text.strip().replace('\n\n', SEP)
    try:
        try:
            r = translator.translate_text(encoded, source_lang='JA', target_lang='ZH-HANT')
        except Exception:
            r = translator.translate_text(encoded, source_lang='JA', target_lang='ZH')
        time.sleep(0.4)
...
def fetch_natalie():
    ...
    # RSS 包含所有近期音樂新聞，_milk_check 在本地過濾（含成員名字）
    arts = _fetch_rss('Natalie Music', 'https://natalie.mu/music/feed/news', limit=10)
    print(f'  -> {len(arts)} articles')
    return arts

def fetch_oricon():
    print('Fetching Oricon...')
    arts = _fetch_rss('Oricon', 'https://www.oricon.co.jp/rss/news/')
    if not arts:
...
def fetch_barks():
    ...
    arts = _fetch_rss('BARKS', 'https://www.barks.jp/rss/')
    if not arts:
...
def fetch_sanspo():
    ...
    arts = _fetch_rss('SANSPO', 'https://www.sanspo.com/rss/')
    if not arts:
...
def fetch_modelpress():
    ...
    arts = _fetch_rss('Modelpress', 'https://mdpr.jp/feed/')
    if not arts:
...
def fetch_setlistfm():
    """setlist.fm から M!LK の今後のライブ日程を取得"""
    ...
    for strong in soup.find_all('strong'):
        ...
        m = DATE_EN.match(text)
        if not m:
            continue
        mon = m.group(1)[:3].capitalize()
        if mon not in _MONTH_MAP:
            continue
        date_str = (f"{m.group(3)}-{_MONTH_MAP[mon]}"
                    f"-{m.group(2).zfill(2)}")
        if date_str < today:
            continue
        container = strong.find_parent(['div', 'article', 'section', 'li'])
        if not container:
            continue
        ...
        link_a  = container.find('a', href=re.compile(r'/setlist/'))
        link    = ('https://www.setlist.fm' + link_a['href']) if link_a else url
    ...
    # Fallback: dateBlock class structure
    if not events:
        for db in soup.select('[class*="dateBlock"]'):
            mon_el  = db.find(class_=re.compile('month', re.I))
            day_el  = db.find(class_=re.compile('day',   re.I))
            year_el = db.find(class_=re.compile('year',  re.I))
            if not (mon_el and day_el and year_el):
                continue
            mon = mon_el.get_text(strip=True)[:3].capitalize()
            if mon not in _MONTH_MAP:
                continue
            date_str = (f"{year_el.get_text(strip=True)}"
                        f"-{_MONTH_MAP[mon]}"
                        f"-{day_el.get_text(strip=True).zfill(2)}")
            if date_str < today:
                continue
            container = db.find_parent(['div', 'article', 'li'])
            venue_a = (container.find('a', href=re.compile(r'/venue/'))
            ...
            link    = ('https://www.setlist.fm' + link_a['href']) if link_a else url
            events.append({
                'id':       make_id((f'[LIVE] {venue}')[:50] + date_str),
                'date':     date_str,
                'title':    f'[LIVE] {venue}' if venue else '[LIVE] M!LK Concert',
                'details':  '',
                'title_zh': '',
    ...
    print(f'  setlist.fm -> {len(events)} upcoming events')
    return events
...
<div id="am-modal" role="dialog" aria-modal="true">
  ...
  <div class="am-scroll">
    ...
    <hr class="am-divider">
    <p class="am-section-label">全文翻譯</p>
...
<script>
function evTgl(id){
  var el=document.getElementById(id),btn=el.querySelector('.tour-btn');
  btn.setAttribute('aria-expanded',el.classList.toggle('open'));
}
</script>
</body>
</html>'''
...
.am-img-item{position:relative;flex-shrink:0;border-radius:8px;overflow:hidden;border:1px solid #e8e8ee;background:#eee;max-width:300px;}
.am-img-item img{display:block;width:100%;height:auto;max-height:200px;object-fit:contain;}
...
def generate_schedule_html(articles, events, updated_at):
    ...
    for name, badge, station, sched, url, color, status, dow in SCHEDULE_PROGRAMS:
        card = _prog_card(name, badge, station, sched, url, color, dow)
        if status == 'active':
...
def main():
    ...
    # Back-fill missing translations for existing articles (max 40 to conserve DeepL quota)
    if translator:
        needs_zh = [a for a in existing if a.get('title_ja') and not a.get('title_zh')][:40]
        if needs_zh:
            print(f'Back-filling {len(needs_zh)} missing translations...')
            for a in needs_zh:
