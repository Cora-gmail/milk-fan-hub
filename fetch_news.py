...
def fetch_article_data(url):
    """Fetch article page once → return (hero_image_url, all_images_list, body_text_ja)."""
    image, images, body = '', [], ''
    try:
...
.card-img{aspect-ratio:16/9;overflow:hidden;background:var(--bg);border-radius:14px 14px 0 0;}
...
.am-img-item{position:relative;flex-shrink:0;width:240px;height:160px;overflow:hidden;border-radius:8px;border:1px solid #e8e8ee;background:#eee;}
.am-img-item img{width:100%;height:100%;object-fit:cover;display:block;}
.am-img-dl{display:block;width:100%;height:100%;text-decoration:none;position:relative;}
.am-dl-btn{position:absolute;bottom:6px;right:6px;background:rgba(0,0,0,.58);color:#fff;font-size:.63rem;padding:3px 9px;border-radius:20px;backdrop-filter:blur(3px);opacity:0;transition:opacity .18s;pointer-events:none;}
.am-img-item:hover .am-dl-btn{opacity:1;}
...
def main():
    ...
    for a in new_raw:
        if a.get('url'):
            img, imgs, body = fetch_article_data(a['url'])
            a['image']  = img
            a['images'] = imgs
    ...
    for g in new_goods:
        if g.get('url') and not g.get('image'):
            img, _, _ = fetch_article_data(g['url'])
            if img:
                g['image'] = img
            time.sleep(0.2)
