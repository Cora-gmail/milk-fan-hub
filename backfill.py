...
from fetch_news import (
    ...
    CHART_BJ_SCANNED_FILE,
    _chart_match, make_id, clean,
    SCHEDULE_FILE, _sdmilk_fetch_body_text, _extract_event_date_from_text,
)
...
def main():
    ...
    parser.add_argument('--backfill-bj-charts',     action='store_true', help='回補 Billboard Japan 週別榜單（配合 --since 指定起始日期）')
    parser.add_argument('--repair-body',             action='store_true', help='重抓最近 90 天 body_ja 空白或內容不完整的文章內文')
    parser.add_argument('--repair-schedule-dates',   action='store_true', help='重新抓 sd-milk 官方 LIVE/MEDIA 事件的文章內文，修正行事曆日期')
    ...
    # ── 行事曆日期修復モード ────────────────────────────────────────────────────────
    if args.repair_schedule_dates:
        print('行事曆日期修復モード：sd-milk 官方 LIVE/MEDIA 事件重新抓內文...')
        _repair_schedule_dates()
        return

    # ── Billboard Japan 回補模式 ──────────────────────────────────────────────────
    if args.backfill_bj_charts:
        print(f'Billboard Japan 回補模式：since={since}')
        _backfill_bj_charts(since=since)
        return
...
def _repair_schedule_dates():
    """Re-fetch article bodies for all source='official' schedule events and correct their dates.

    Targets events where the stored date looks like a publication date (not the actual
    broadcast/event date). Fetches each article page, re-extracts the event date, and
    updates schedule.json when a different (better) date is found.
    """
    events = load_json(SCHEDULE_FILE)
    if not events:
        print('  schedule.json が空です。')
        return

    targets = [e for e in events if e.get('source') == 'official' and e.get('url')]
    print(f'  対象: {len(targets)} 件の official イベント')

    fixed = 0
    for ev in targets:
        url      = ev['url']
        title    = ev.get('title', '')
        pub_date = ev.get('date', '')
        body_text = _sdmilk_fetch_body_text(url)
        if not body_text:
            time.sleep(0.3)
            continue
        new_date = _extract_event_date_from_text(title, body_text, pub_date)
        if new_date and new_date != pub_date:
            print(f'  {pub_date} → {new_date}  {title[:60]}')
            ev['date'] = new_date
            fixed += 1
        time.sleep(0.4)
...
def _backfill_bj_charts(since: str):
    """Back-fill Billboard Japan weekly charts from `since` date to today."""
    ...
    BJ_BASE = 'https://www.billboard-japan.com/charts/detail'
    bj_headers = dict(HEADERS, **{'Accept-Language': 'ja,en;q=0.9'})

    since_dt  = _date.fromisoformat(since)
    today_dt  = _date.today()

    # Generate all Mondays from since_dt to today
    cur = since_dt
    while cur.weekday() != 0:
        cur += _td(days=1)
    mondays = []
    ...
    print(f'  Mondays to scan: {len(mondays)}  ({mondays[0]} ~ {mondays[-1]})')

    for chart_code, chart_name in BJ_CHARTS:
        found_code = 0
        for date_dt in mondays:
            date_str = date_dt.strftime('%Y-%m-%d')
            if (chart_code, date_str) in scanned_pairs:
                continue
            url = (f'{BJ_BASE}?a={chart_code}'
                   f'&year={date_dt.year}'
            ...
            try:
                ...
                for item in items:
                    ...
                    if entry['id'] not in ex_ids:
                        ex_ids.add(entry['id'])
                        new_entries.append(entry)
                        found_code += 1
                newly_scanned.append({'chart_code': chart_code, 'date': date_str})
                scanned_pairs.add((chart_code, date_str))
            except Exception as e:
                print(f'  [BJ {chart_code} {date_str}] {e}')
            _time.sleep(0.8)
        print(f'  [{chart_name}] {found_code} 件追加')

    if newly_scanned:
        combined = ex_bj_scanned + [s for s in newly_scanned
                                     if (s['chart_code'], s['date']) not in
                                     {(e['chart_code'], e['date']) for e in ex_bj_scanned}]
        save_json(CHART_BJ_SCANNED_FILE, combined)

    if new_entries:
        merged = new_entries + existing_charts
        save_json(CHARTS_FILE, merged)
        print(f'\nBillboard Japan 回補完了: {len(new_entries)} 件追加 (合計 {len(merged)} 件)')
    else:
        print('\n追加する Billboard Japan エントリはありませんでした。')
...
def _backfill_charts(since: str, weekly: bool = True, daily: bool = True):
    ...
    if weekly:
        ...
        # Generate all Mondays from since_dt to today
        # Oricon weekly chart date = Monday (chart published Monday morning JST)
        cur = since_dt
        while cur.weekday() != 0:  # advance to first Monday
            cur += _td(days=1)
        sundays = []
        while cur <= today_dt:
            ...
            for date_str in dates:
                ...
                for e in entries:
                    if e['id'] not in ex_ids:
                        ...
                        new_entries.append(e)
                        found_code += 1
                if page_ok:  # only mark scanned when HTTP 200; 404/timeout will be retried
