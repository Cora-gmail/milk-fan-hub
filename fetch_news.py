...
def fetch_gnews_schedule_events():
    ...
    _KW = {'ライブ', 'コンサート', '出演', '公演', 'ツアー', '放送', '特番', 'イベント',
           'fanmeet', 'tour', 'concert', 'live', 'event', 'ファンミ'}
    queries = [
        '"M!LK" (ライブ OR コンサート OR 公演 OR ツアー OR ファンミ)',
        '"M!LK" (出演 OR 放送 OR 特番 OR イベント)',
        '("吉田仁人" OR "佐野勇斗" OR "山中柔太朗" OR "塩﨑太智" OR "曽野舜太") (出演 OR 放送 OR ライブ)',
    ]
    events, seen = [], set()
...
def _render_today_schedule(events, today_dow, today_str):
    ...
    for name, badge, station, sched, url, color, status, dow_list, members in SCHEDULE_PROGRAMS:
        ...
        effective_dow = dow_list if dow_list is not None else _prog_dow_cache.get(name)
        if effective_dow is not None and today_dow in effective_dow:
