...
CHART_SCANNED_FILE  = 'chart_scanned.json'   # (chart_code, date) pairs already processed
MAX_ARTICLES  = 1000
    ...
    for base_url, cat_label in CAT_URLS:
        ...
        for page in range(1, 5):
            ...
            try:
                ...
                soup = BeautifulSoup(r.text, 'html.parser')
                article_urls = []
                # Try __NEXT_DATA__ first (Next.js SSR — more reliable than CSS selectors)
                nd = _next_data_articles(soup)
                for na in nd:
                    href = na.get('url', '')
                    if href and re.search(r'/contents/\d+', href):
                        full_url = href if href.startswith('http') else 'https://sd-milk.com' + href
                        if full_url not in seen:
                            seen.add(full_url)
                            article_urls.append(full_url)
                # Fallback: CSS li selector
                if not article_urls:
                    for li in soup.select('li'):
                        a = li.find('a', href=True)
                        if not a:
...
def generate_chart_html(charts, updated_at):
    ...
    recent   = [c for c in charts if c['date'] >= year_start]   # 今年
    historic = [c for c in charts if c['date'] <  year_start]   # 去年以前
...
def generate_schedule_html(articles, events, updated_at, official_news=None):
    ...
    prog_broadcast_html = (
        ...
        if prog_cards_list
            ...
            else '<p class="today-empty">現在放送中のTV・ラジオ番組はありません。</p>'
        )
    )
    prog_works_html = (
...
def main():
    ...
    for _a in existing:
        ...
        if (_a.get('date', '') or '')[:10] < _body_cutoff:
            continue
        _img, _, _body, _ = fetch_article_data(_a['url'])
        if _body:
            _a['body_ja'] = _body
            _body_repaired += 1
    ...
    for _a in existing:
        ...
        if (_a.get('date', '') or '')[:10] < _img_cutoff:
            continue
        _img, _, _, _ = fetch_article_data(_a['url'])
        if _img:
            _a['image'] = _img
            _img_repaired += 1
        time.sleep(0.3)
    ...
    # Persist newly scanned (chart_code, date) pairs to avoid re-scanning
    if newly_scanned:
        ...
        # Trim to last 52 weeks to avoid unbounded growth
