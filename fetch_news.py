...
def translate(text):
    ...
    # DeepL（優先、quota 切れたら即 fallback）
    if translator and not _deepl_quota_exceeded:
        try:
            try:
                r = translator.translate_text(chunk, source_lang='JA', target_lang='ZH-HANT')
            except Exception:
                r = translator.translate_text(chunk, source_lang='JA', target_lang='ZH')
            time.sleep(0.4)
            return r.text.replace(SEP, '\n\n')
        except Exception as e:
            msg = str(e)
            if 'Quota' in msg or 'quota' in msg or '456' in msg:
                _deepl_quota_exceeded = True
                print('  DeepL quota exhausted — switching to Google Translate fallback')
            else:
                print(f'  DeepL error: {e}')

    # Google Translate fallback（無需 API key）
    if _google_translate_ok:
        try:
            result = _GoogleTranslator(source='ja', target='zh-TW').translate(chunk)
            time.sleep(0.3)
            return (result or '').replace(SEP, '\n\n')
        except Exception as e:
...
def fetch_article_data(url):
    ...
    try:
        ...
        body = content_el  # full body, no character limit
...
def fetch_oricon():
    ...
    # ① Google News RSS（CF bypass、"M!LK" site:oricon.co.jp 専用クエリ）
    #    Cloudflare 保護のため Oricon 直接スクレイプ不可、Google News 経由で取得
...
SCHEDULE_PROGRAMS = [
    # (name, badge, station, sched, url, color, status, dow_list)
    # dow_list: 放送曜日 0=月 1=火 2=水 3=木 4=金 5=土 6=日, None=不明
    # ── 現在放送中 ──
    ('やってM!LK',
     'TV', 'TBS テレビ', '定期放送中',
     'https://www.tbs.co.jp/yatte_milk/', BADGE_COLORS['TV'], 'active', None),
    ('レコメン!',
     'ラジオ', '文化放送 毎週月〜金', '定期放送中',
     'https://www.joqr.co.jp/qr/program/reco/', BADGE_COLORS['ラジオ'], 'active', [0,1,2,3,4]),
    ('イマドキッ ドゥフドゥフナイト',
     'ラジオ', 'MBSラジオ', '定期放送中',
     'https://www.mbs1179.com/imadoki/', BADGE_COLORS['ラジオ'], 'active', None),
    # ── 公開予定・近日 ──
    ('君の好きは無敵',
     'TV', 'TBS テレビ 火曜22:00〜', '放送予定',
     'https://www.tbs.co.jp/kiminosukihamuteki_tbs/', BADGE_COLORS['TV'], 'upcoming', [1]),
    ('トイ・ストーリー５',
     '映画', 'Disney / 2026年夏公開予定', '公開予定',
     'https://www.disney.co.jp/movie/toy5', BADGE_COLORS['映画'], 'upcoming', None),
    ('仮面ライダーゼッツ＆超宇宙刑事ギャバン',
     '映画', '2026年夏公開予定', '公開予定',
     'https://zeztz-gavan-26movie.com/', BADGE_COLORS['映画'], 'upcoming', None),
    # ── リリース ──
    ('M!LK 1st Mini Album（タイトル未定）',
     'リリース', '2026年09月16日発売', '予約受付中',
     'https://victor-store.jp/', BADGE_COLORS['リリース'], 'release', None),
]
...
def fetch_sd_milk_programs_pw():
    ...
    programs = []
    TYPE_KW = {
        'TV':      ['テレビ', 'ドラマ', 'バラエティ', 'tv', 'tbs', 'フジ', 'ntv', '日テレ', 'テレ朝', 'abc'],
...
function openArticleModal(a){
  ...
  function splitParas(t){var ps=t.split(/\n\n+/);if(ps.length<=1)ps=t.split(/\n/);return ps.map(function(p){return p.trim()?'<p>'+esc(p.trim())+'</p>':'';}).join('');}
...
/* ── schedule calendar ──────────────────────────────────────────────────── */
.sc-cal-wrap{background:var(--card);border:1px solid var(--border);border-radius:16px;padding:18px;box-shadow:var(--shadow);}
.sc-cal-nav{display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;}
.sc-cal-title{font-size:.95rem;font-weight:800;color:var(--text);}
.sc-cal-arrow{background:none;border:1px solid var(--border);color:var(--muted);width:28px;height:28px;border-radius:7px;cursor:pointer;font-size:.85rem;display:flex;align-items:center;justify-content:center;transition:border-color .2s,color .2s;}
...
.sc-cal-dow{font-size:.58rem;font-weight:700;text-align:center;color:var(--muted);letter-spacing:.04em;padding:3px 0;}
.sc-cal-cell{aspect-ratio:1;display:flex;flex-direction:column;align-items:center;justify-content:center;border-radius:7px;cursor:default;position:relative;font-size:.75rem;color:var(--muted);}
...
.sc-cal-dot{display:flex;gap:2px;margin-top:2px;flex-wrap:wrap;justify-content:center;}
.sc-dot-live{width:5px;height:5px;border-radius:50%;background:var(--red);flex-shrink:0;}
.sc-dot-ev{width:5px;height:5px;border-radius:50%;background:var(--blue);flex-shrink:0;}
.sc-day-events{margin-top:14px;display:none;}
.sc-day-events.visible{display:block;}
.sc-day-title{font-size:.72rem;font-weight:700;color:var(--blue);margin-bottom:8px;letter-spacing:.1em;}
.sc-evt-item{display:flex;align-items:flex-start;gap:9px;padding:8px 0;border-bottom:1px solid var(--border);}
...
__LIVE_SECTION__
  ...
  <div class="sched-section">
    <p class="sched-section-title">&#127914; 出演番組・作品</p>
    __PROG_CARDS__
  </div>
  ...
  <div class="sched-section">
    ...
    <div class="sc-cal-wrap">
      ...
      <div class="sc-day-events" id="sc-day-events">
        <p class="sc-day-title" id="sc-day-title"></p>
  ...
  <div class="sched-section">
    <p class="sched-section-title">&#128190; 過去出演 &#38957;&#27511;</p>
    __PAST_EVENTS__
  </div>
...
function scShowDay(date){
  ...
  evts.forEach(function(e){
    ...
    item.innerHTML='<div class="sc-evt-dot" style="background:'+dotColor+'"></div>'
      +'<div class="sc-evt-body"><p class="sc-evt-title">'+(e.title||'').replace('[LIVE]','').trim()+'</p>'
      +(detail?'<p class="sc-evt-detail">'+detail+'</p>':'')+'</div>';
    if(e.url&&e.url!=='#'){
...
function scCalPrev(){if(scM===0){scY--;scM=11;}else{scM--;}scSel='';scRender();document.getElementById('sc-day-events').classList.remove('visible');}
function scCalNext(){if(scM===11){scY++;scM=0;}else{scM++;}scSel='';scRender();document.getElementById('sc-day-events').classList.remove('visible');}
...
def _render_today_schedule(events, today_dow, today_str):
    ...
    airing = []
    for name, badge, station, sched, url, color, status, dow_list in SCHEDULE_PROGRAMS:
        if status != 'active':
            continue
...
def generate_schedule_html(articles, events, updated_at):
    ...
    def _prog_card(name, badge, station, sched, url, color, dow=None):
    ...
    TYPE_COLOR = {'TV': '#b71c1c', 'ラジオ': '#0277bd', '映画': '#37474f',
                  'イベント': '#00695c', 'リリース': '#6a1b9a'}

    # Index hardcoded programs by name for supplement lookup
    ...
    if _dynamic_programs:
        # Primary source: sd-milk; supplement schedule/color/dow from hardcoded where available
        for p in _dynamic_programs[:30]:
            p_name   = p.get('name', '')
            p_type   = p.get('type', 'TV')
            p_status = p.get('status', 'upcoming')
            color    = TYPE_COLOR.get(p_type, '#555')
            hc = hc_lookup.get(p_name)
            if hc:
                _, hc_badge, hc_station, hc_sched, hc_url, hc_color, _, hc_dow = hc
                ...
                dow      = None
            card = _prog_card(p_name, p_type, station, sched_display, url_use, color, dow)
            seen_names.add(p_name)
            if p_status == 'active':
                active_cards.append(card)
            elif p_status == 'release':
                release_cards.append(card)
            else:
                upcoming_cards.append(card)

        # Supplement: hardcoded items not returned by sd-milk
        for name, badge, station, sched, url, color, status, dow in SCHEDULE_PROGRAMS:
            if name in seen_names:
                continue
            sched_display = _prog_sched_cache.get(name, sched)
            card = _prog_card(name, badge, station, sched_display, url, color, dow)
            if status == 'active':
                active_cards.append(card)
            elif status == 'release':
                release_cards.append(card)
    ...
    if active_cards:
        prog_sections += (
            '<p class="sched-section-label">&#127897; 現在放送中・出演中</p>'
            '<div class="prog-grid">' + ''.join(active_cards) + '</div>'
        )
    if upcoming_cards:
        prog_sections += (
            '<p class="sched-section-label" style="margin-top:28px">&#127916; 公開予定</p>'
            '<div class="prog-grid">' + ''.join(upcoming_cards) + '</div>'
        )
    if release_cards:
        prog_sections += (
            '<p class="sched-section-label" style="margin-top:28px">&#127926; リリース予定</p>'
            '<div class="prog-grid">' + ''.join(release_cards) + '</div>'
        )
    prog_cards = [prog_sections]  # kept as list for template compatibility
    dyn_html = ''  # merged into prog_cards; placeholder kept for template compat

    # ── Past events section ─────────────────────────────────────────────────
    past_events = sorted(
        [e for e in events if e.get('date', '') < today_jst],
    ...
    # ── Schedule calendar data (all events as JSON) ─────────────────────────
    import json as _json
