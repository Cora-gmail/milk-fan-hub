...
SCHEDULE_PROGRAMS = [
    ...
    ('君の好きは無敵',
     ...
     'https://www.tbs.co.jp/kiminosukihamuteki_tbs/', BADGE_COLORS['ドラマ'], 'upcoming', [1]),
    ('トイ・ストーリー５',
...
function openArticleModal(a){
  ...
  function splitParas(t){
    ...
    if(ps.length<=1&&t.length>150){
      // fallback：以句號為段落分界
      t=t.replace(/([。！？])\s*/g,'$1\n');
      ps=t.split(/\n/);
    }
    return ps.filter(function(p){return p.trim().length>0;}).map(function(p){return '<p>'+esc(p.trim())+'</p>';}).join('');
  }
...
def generate_schedule_html(articles, events, updated_at):
    ...
    prog_cards_list = []   # 出演番組
    work_cards_list = []   # 放送中作品
    seen_norms      = set()

    def _add_card(p_name, p_type, station, sched_d, url_use, color, dow):
        card = _prog_card(p_name, p_type, station, sched_d, url_use, color, dow)
    ...
    if _dynamic_programs:
        for p in _dynamic_programs[:40]:
            ...
            _add_card(p_name, p_type, station, sched_display, url_use, color, dow)
        ...
        for name, badge, station, sched, url, color, status, dow in SCHEDULE_PROGRAMS:
            ...
            _add_card(name, badge, station, sched_display, url, color, dow)
    else:
    ...
    # ── Schedule calendar data (events + dynamic program dates) ──────────────
    import json as _json
    cal_events = list(events)
    for p in _dynamic_programs:
        sd = p.get('start_date', '')
        if sd and sd >= today_jst[:4]:  # only current year or later
            cal_events.append({
                'date':    sd,
                'title':   p.get('name', ''),
