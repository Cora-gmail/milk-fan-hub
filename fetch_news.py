...
def fetch_oricon():
    ...
    # ② Oricon 公式 RSS 補完（general feed、limit 大きめ）
    rss_arts = _fetch_rss('Oricon', 'https://www.oricon.co.jp/rss/news/', limit=30)
    for a in rss_arts:
        u = a.get('url', '')
        if u and u not in seen:
            seen.add(u)
    ...
    print(f'  -> {len(arts)} articles (gnews:{len(gnews_arts)} rss:{len(rss_arts)})')
    return arts
...
def main():
    ...
    # （例: "M!LK... - BARKS" → "M!LK..."、title_zh もリセットして再翻訳）
    _fixed_titles = 0
