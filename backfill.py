...
from fetch_news import (
    HEADERS, JST, _MEMBER_NAMES, _milk_check,
    normalize_date, make_id, clean, translate,
...
def _scrape_musicvoice(since: str, max_pages: int):
    ...
    for page in range(1, max_pages + 1):
        ...
        for a in soup.find_all('a', href=True):
            href = a['href']
            if 'musicvoice.jp' not in href or '/20' not in href:
                continue
            title = a.get_text(strip=True)
            if not title or not _milk_check(title):
                continue

            # 從 URL 或父元素找日期
            date_str = ''
            dm = re.search(r'/(20\d\d)/(\d{2})/', href)
            if dm:
                date_str = f'{dm.group(1)}-{dm.group(2)}-01'
            # try sibling / parent text
            parent_text = a.find_parent().get_text() if a.find_parent() else ''
            dm2 = re.search(r'(20\d\d)[.\-/](\d{2})[.\-/](\d{2})', parent_text)
            if dm2:
            ...
            art = _make_article('Musicvoice', title, href, date_str, '')
            articles.append(art)
            found += 1
...
def main():
    ...
    merged, added = merge_by_url(existing, raw)
    print(f'新增 {len(added)} 筆（重複 {len(raw)-len(added)} 筆跳過）')
    ...
    for i, a in enumerate(added):
        if a.get('url'):
            try:
                ...
                print(f'  [{i+1}/{len(added)}] 抓取失敗: {e}')
            print(f'  [{i+1}/{len(added)}] {a.get("date","?"):10s} {a["title_ja"][:45]}...')
            time.sleep(0.4)
    ...
    # 寫回
    save_json(ARTICLES_JSON, merged)
    print(f'\n已寫入 {ARTICLES_JSON}，共 {len(merged)} 篇。')
