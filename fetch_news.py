...
SOURCE_COLORS['FC限定 (STARDUST)'] = '#9c27b0'
...
def generate_goods_html(goods, articles, updated_at):
    ...
    # ── Section 1: リリース情報 (from news articles) ──────────────────────────
    releases = [
        a for a in articles
        if any(kw in (a.get('title_ja', '') + a.get('summary_ja', '')).lower()
               for kw in RELEASE_KW)
    ]
    releases = sorted(releases, key=lambda a: a.get('date', ''), reverse=True)[:12]
    if releases:
        cards = ''.join(_release_card_html(a, esc) for a in releases)
        sections.append(
            f'<div class="goods-section">'
            f'<p class="section-label">&#127926; リリース情報'
            f'<span class="cnt">({len(releases)})</span></p>'
            f'<div class="release-grid">{cards}</div>'
            f'</div>'
        )
    ...
    if fc_goods:
        ...
        sections.append(
            ...
            f'<div class="goods-empty">'
            f'<a href="https://store.plusmember.jp/stardustch/products/list.php?category_id=891"'
            f' target="_blank" rel="noopener">FC限定ショップを確認する &rarr;</a>'
            f'</div></div>'
        )
    ...
    if official:
        ...
        sections.append(
            ...
            f'<div class="goods-empty">'
            f'<a href="https://sd-milk.com/goods" target="_blank" rel="noopener">'
            f'公式サイトで確認する &rarr;</a>'
            f'</div></div>'
        )
