...
def fetch_article_data(url):
    ...
    # Google News redirect URLs use JS redirects; requests only gets the intermediate page
    # whose og:image is the Google News logo — skip fetching entirely for these URLs.
...
def _fetch_rss(source, feed_url, limit=8):
    ...
    try:
        ...
        for e in feed.entries:
            title   = e.get('title', '')
            # Google News RSS appends "- SOURCE NAME" to every title; strip it
            if _is_gnews:
...
def main():
    ...
    # （Google News 中間ページの og:image は googleusercontent.com 等に格納されるため
    #   画像 URL ではなく記事 URL で判定する）
    _fixed = 0
    for _a in existing:
        if 'news.google.com' in _a.get('url', '') and _a.get('image'):
            _a['image'] = ''
            _fixed += 1
    if _fixed:
        print(f'  Cleared {_fixed} Google News logo images from existing articles')

    # Google News RSS タイトルに付いた "- SOURCE NAME" を過去記事から除去
    # URL が news.google.com の記事のみ対象（他ソースの正当な " - サブタイトル" を誤って消さないため）
    _fixed_titles = 0
    ...
    # Back-fill missing translations for existing articles (max 200 to cover large gaps)
    if translator or _google_translate_ok:
        ...
        for _a in existing:
            ...
            if _a.get('title_ja') and not _a.get('title_zh'):
                _a['title_zh'] = translate(_a['title_ja'])
                _tx_count += 1
        ...
        for _a in existing:
            ...
            if _a.get('summary_ja') and not _a.get('summary_zh'):
                _a['summary_zh'] = translate(_a['summary_ja'])
                _tx_count += 1
        if _tx_count:
