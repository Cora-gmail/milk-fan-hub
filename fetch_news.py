...
def merge_by_url(existing, new_items):
    ...
    added = [a for a in new_items if a.get('url') and a['url'] not in seen]
...
def fetch_tvguide():
    ...
    # M!LK has a dedicated keyword page on TVガイドWeb
...
def fetch_schedule():
    ...
    events = []
    for path in ['/schedule/', '/live/', '/event/', '/liveschedule/', '/']:
        try:
            r = requests.get(f'https://sd-milk.com{path}', headers=HEADERS, timeout=12)
            if r.status_code != 200:
                continue
            ...
            for sel in [
                ...
                'table.schedule tr', '.schedule-wrap li',
                '.live-schedule li', '.liveschedule li', '.info-list li',
            ]:
                items = soup.select(sel)
                if len(items) < 2:
                    continue
                for it in items:
                    ...
                    if date_el:
                        ...
                        date_str = (f"{m.group(1)}-{m.group(2).zfill(2)}-{m.group(3).zfill(2)}"
                                    if m else datetime.now(JST).strftime('%Y-%m-%d'))
...
.card{background:var(--card);border:1px solid var(--border);border-radius:14px;overflow:hidden;display:flex;flex-direction:column;transition:transform .2s,box-shadow .2s,border-color .2s;box-shadow:var(--shadow);}
.card:hover{transform:translateY(-3px);box-shadow:var(--shadow-hover);border-color:#b4caff;}
.card-img{position:relative;padding-top:56.25%;overflow:hidden;background:var(--bg);}
.card-img img{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;}
.img-dl{position:absolute;bottom:6px;right:6px;background:rgba(42,91,215,.85);color:#fff;font-size:.63rem;padding:3px 8px;border-radius:10px;text-decoration:none;backdrop-filter:blur(4px);transition:background .2s;}
.img-dl:hover{background:var(--blue);}
...
footer{text-align:center;padding:22px;font-size:.7rem;color:var(--muted);border-top:1px solid var(--border);background:var(--surface);}
'''
...
SOURCES_HTML = (
    '<div class="sources">'
    '<span style="background:#d4006e">sd-milk.com</span>'
    '<span style="background:#0066aa">Natalie</span>'
    '<span style="background:#cc2233">Oricon</span>'
    '<span style="background:#b05010">BARKS</span>'
    '<span style="background:#0a6b30">SANSPO</span>'
    '<span style="background:#1a4a9f">Billboard Japan</span>'
    '<span style="background:#7b1ea2">Musicvoice</span>'
    '<span style="background:#d45500">SPICE</span>'
    '<span style="background:#1b5e20">Musicman</span>'
    '<span style="background:#c62828">The First Times</span>'
    '<span style="background:#1565c0">Modelpress</span>'
    '<span style="background:#e91e8c">美的.com</span>'
    '<span style="background:#0d47a1">MEN\'S NON-NO</span>'
    '<span style="background:#2c2c2c">VOGUE JAPAN</span>'
    '<span style="background:#e53935">TVガイドWeb</span>'
    '<span style="background:#00838f">WEBザテレビジョン</span>'
    '<span style="background:#6d4c41">ebidan.jp</span>'
    '<span style="background:#cc0000">やってM!LK</span>'
    '<span style="background:#880033">君の好きは無敵</span>'
    '<span style="background:#0055bb">レコメン!</span>'
    '<span style="background:#e65100">イマドキッ</span>'
    '<span style="background:#0d47a1">トイ・ストーリー５</span>'
    '<span style="background:#1b5e20">仮面ライダーゼッツ</span>'
    '</div>'
)
...
<footer>m!lk Fan Hub &nbsp;&middot;&nbsp; 非公式 / 非官方 &nbsp;&middot;&nbsp; 毎日 09:00 JST 自動更新</footer>
<script>
...
function renderCalendar(){
  ...
  for(var d=1;d<=dt;d++){
    ...
    btn.className='cal-day'+(datesWithNews.has(ds)?' has-news':'')+(selected===ds?' selected':'')+(ds===ts?' today':'');
    btn.textContent=d;
...
function makeCard(a){
  ...
  var imgHtml='';
  if(a.image){
    imgHtml='<div class="card-img"><img src="'+esc(a.image)+'" alt="" loading="lazy" onerror="this.closest(\'.card-img\').style.display=\'none\'">' +
      '<a class="img-dl" href="'+esc(a.image)+'" target="_blank" rel="noopener">&#8599; 原圖</a></div>';
  }
...
<footer>m!lk Fan Hub &nbsp;&middot;&nbsp; 演出情報參考自 sd-milk.com &nbsp;&middot;&nbsp; 毎日 09:00 JST 自動更新</footer>
<script>
