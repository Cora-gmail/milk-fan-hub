...
CHART_SCANNED_FILE       = 'chart_scanned.json'        # weekly (chart_code, date) pairs
CHART_DAILY_SCANNED_FILE = 'chart_daily_scanned.json'  # daily  (chart_code, date) pairs
CHART_BJ_SCANNED_FILE    = 'chart_bj_scanned.json'     # Billboard Japan (chart_code, date) pairs
MAX_ARTICLES  = 1000
...
def translate(text):
    ...
    # Google Translate fallback — translate paragraph by paragraph to preserve structure
    if _google_translate_ok:
        ...
        translated = []
        for p in paras:
            # Google Translate has ~5000 char/request limit; chunk long paragraphs
            chunks = []
            if len(p) > 4000:
                buf = ''
                for sent in re.split(r'([。！？\n])', p):
                    if len(buf) + len(sent) > 4000 and buf:
            ...
            for chunk in chunks:
                try:
                    result = _GoogleTranslator(source='ja', target='zh-TW').translate(chunk)
                    chunk_results.append(result or chunk)
                    time.sleep(0.25)
                except Exception as e:
                    print(f'  Google Translate error: {e}')
                    chunk_results.append(chunk)
            translated.append(''.join(chunk_results))
        return '\n\n'.join(translated)
...
_ARTICLE_BODY_SELECTORS = [
    ...
    '.pg-articleDetail__body',                       # Modelpress (mdpr.jp)
    '.pg-article__body', '.pg-articleBody',          # Modelpress 新版
    '.block-detail-body',                            # Oricon News（.mod-pより上位で全段落を取得）
    '.articleBody', '.article__body',                # 通用變體
    '.atpress-body', '.pr-body', '.release-body',    # atpress / press release
    '.article-detail__body', '.article-detail__text', # Sanspo
    '.article__text', '.article-text__body',          # Sanspo / Musicvoice variants
    '.barks-article-body', '.barks-article__body',    # Barks
    '.spice-article-body', '.detail-box',             # SPICE / eplus
    # 一般 selector
...
# Photo-gallery page patterns across all supported sources
_PHOTO_URL_PATTERNS = [
    r'mdpr\.jp/photo/',                            # Modelpress photo pages
    r'natalie\.mu/[^/]+/photo/',                   # Natalie Music
    r'barks\.jp/.+/photo/?$',                      # Barks
    r'spice\.eplus\.jp/.+/photo/?(\?|$)',          # SPICE
    r'musicman\.co\.jp/.+/photo',                  # Musicman
    r'thefirsttimes\.jp/.+/photo',                 # The First Times
    r'oricon\.co\.jp/.+/photo/\d',                 # Oricon news photo sub-pages (/news/xxx/photo/N/)
    r'oricon\.co\.jp/photo/',                      # Oricon photo path
    r'sanspo\.com/.+/photo/',                      # Sanspo photo sub-pages
    r'/gallery/\d',                                # Generic /gallery/NNN
    r'/photo/[A-Z0-9]{12,}',                       # Hash-based photo slugs (SANSPO style)
]
_PHOTO_URL_RE = re.compile('|'.join(_PHOTO_URL_PATTERNS), re.IGNORECASE)
_PHOTO_TITLE_RE = re.compile(r'^(画像|写真|フォト)[・・\s【]')

def _is_photo_page(url, title=''):
    """Return True if this URL / title is a photo-gallery sub-page rather than a news article."""
    if _PHOTO_TITLE_RE.match(title or ''):
        return True
    return bool(_PHOTO_URL_RE.search(url or ''))
...
def fetch_article_data(url):
    """Fetch article page once → return (hero_image_url, all_images_list, body_text_ja, pub_date, resolved_url)."""
    ...
    if 'news.google.com' in url:
        ...
        if not real_url:
            return image, images, body, pub_date, url
        url = real_url  # fetch the actual article instead
    # Oricon: always fetch the /full/ version for complete article body
    if 'oricon.co.jp' in url:
        url = _norm_oricon_url(url)
    # Modelpress: preview page hides body; /detail/ URL has the full article
    if 'mdpr.jp' in url:
        url = _norm_mdpr_url(url)
    try:
        ...
        # Return True if the element's visible text is mostly anchor link text
        # (related-article lists, recommendation widgets, etc.)
        def _link_heavy(tag_el):
            full = tag_el.get_text().strip()
            if not full:
                return True
            link_text = ''.join(a.get_text() for a in tag_el.find_all('a'))
            return (len(link_text) / len(full)) > 0.65

        # Strip sidebar/related/ad elements before reading body text
        _BODY_NOISE_SEL = (
            'script,style,nav,aside,header,footer,'
            '.sns,.share,.related,.related-articles,.related-posts,'
            '.recommend,.recommended,.pickup,.ranking,'
            '[class*="related"],[class*="recommend"],[class*="pickup"],'
            '[class*="link-list"],[class*="linkList"],[class*="link_list"],'
            '[class*="more-article"],[class*="next-article"],[class*="prev-article"],'
            '[class*="sidebar"],[class*="side-bar"],[class*="widget"],'
            '[class*="ad-"],[class*="-ad"],[class*="advertisement"],[class*="sponsor"],'
            '.tag,.tags,.category-list,.breadcrumb,.pagination,'
            '.author,.author-info,.profile,'
            'figure figcaption,.caption,.img-caption'
        )
        ...
        for sel in _ARTICLE_BODY_SELECTORS:
            ...
            if el:
                ...
                el = _copy.copy(el)  # don't mutate the original soup
                for tag in el.select(_BODY_NOISE_SEL):
                    tag.decompose()
                # 以 <p>/<h2>/<h3>/<li> 為段落分界；若無則用 <br> 切割（Oricon 等用 br 換行）
                blocks = []
                for child in el.find_all(['p', 'h2', 'h3', 'li']):
                    if _link_heavy(child):  # skip related-article link lists
                        continue
                    t = re.sub(r'\s+', ' ', child.get_text()).strip()
                    if len(t) > 10:
                        blocks.append(t)
                if not blocks:
                    # br-based: get_text with separator
                    raw = el.get_text(separator='\n').strip()
                    blocks = [re.sub(r'\s+', ' ', ln).strip()
                              for ln in raw.splitlines() if len(ln.strip()) > 10]
                if blocks:
                    text = '\n\n'.join(blocks)
                else:
                ...
                if len(text) > 80:
                    ...
                    break
        if not content_el:
            # Fallback: gather <p> from whole page, skip link-heavy ones
            paras = []
            for p in soup.find_all('p'):
                if len(p.get_text(strip=True)) > 40 and not _link_heavy(p):
                    paras.append(re.sub(r'\s+', ' ', p.get_text()).strip())
            content_el = '\n\n'.join(paras) if paras else ''

        # ── 過濾雜訊段落（版權聲明、URL、製作人員表、関連記事ヘッダ等）──
        _NOISE_RE = re.compile(
            r'https?://'
            r'|©\s*\d{4}'
            r'|\(C\)\s*\d{4}'
            r'|^[\s　]*(?:導演|監督|製作|配音|発売|発行|出版|©|Copyright)[：:：]'
            r'|^[\s　]*(?:関連記事|おすすめ記事|あわせて読みたい|合わせて読みたい)'
            r'|^[\s　]*(?:もっと見る|続きを読む|詳細はこちら|詳しくはこちら)'
            r'|^[\s　]*(?:この記事|この写真|フォト|写真\d+枚)'
            r'|\bPR\b|\bAD\b|広告|スポンサー',
            re.IGNORECASE
        )
        clean_paras = []
        for line in content_el.split('\n\n'):
            ...
            if _NOISE_RE.search(stripped):
                continue
            # Skip very short lines with no Japanese punctuation (likely section labels or tags)
            if len(stripped) < 12 and not re.search(r'[。、！？…『』「」]', stripped):
                continue
            clean_paras.append(stripped)
        content_el = '\n\n'.join(clean_paras)

        body = content_el[:4000]  # expanded limit for complete articles
        ...
        # ── Oricon photo gallery: iterate /photo/N/ sub-pages for full-res images ──
        if 'oricon.co.jp' in url:
            base = re.sub(r'/full/?$', '', url.rstrip('/'))
            _seen_ph = set(images)
            for _i in range(1, 13):  # up to 12 sub-pages
                _photo_url = f'{base}/photo/{_i}/'
                try:
                    _pr = requests.get(_photo_url, headers=HEADERS, timeout=8)
                    if _pr.status_code != 200:
                        break
                    _psoup = BeautifulSoup(_pr.text, 'html.parser')
                    _og = _psoup.select_one('meta[property="og:image"]')
                    if _og:
                        _ph = _og.get('content', '').strip()
                        if _ph and _ph not in _seen_ph and not any(w in _ph.lower() for w in _SKIP_IMG_WORDS):
                            _seen_ph.add(_ph)
                            if not image:
                                image = _ph
                            images.append(_ph)
...
def _norm_url(url):
    ...
    try:
        ...
        for k in ['ref', 'utm_source', 'utm_medium', 'utm_campaign', 'utm_content',
            ...
            params.pop(k, None)
        path = p.path.rstrip('/')
        # Strip photo sub-page suffixes so article + its photo pages share one URL
        path = re.sub(r'/photo/[A-Z0-9]{12,}$', '', path, flags=re.IGNORECASE)
        if 'oricon.co.jp' in p.netloc:
            path = re.sub(r'/p/\d+', '', path)
            path = re.sub(r'/photo/\d+', '', path)
            if '/full' in path:
                path = re.sub(r'/full.*', '/full', path)
        if 'mdpr.jp' in p.netloc:
            # Normalize /category/id → /category/detail/id for dedup
            path = re.sub(r'^(/[^/]+)/(\d+)$', r'\1/detail/\2', path)
        q = urlencode(params, doseq=True)
...
def _milk_check(text):
    t = text.lower()
    if not ('m!lk' in t or 'ミルク' in text or any(m in text for m in _MEMBER_NAMES)):
        return False
    # Reject: M!LK listed as one of many artists ("超特急、M!LKなど")
    # Allow through only if M!LK is also a direct syntactic subject (M!LKが/の/は…)
    if (re.search(r'M!LKなど', text) and
            not re.search(r'M!LK(?:が|の|は|を|で|に|と|より|として|だけ)', text)):
        return False
    return True
...
# Space-stripped versions for flexible matching (full-width / half-width space variants)
_CHART_MATCH_NOSPACE = frozenset(re.sub(r'[\s　]', '', n) for n in _CHART_MATCH_NAMES)

def _chart_match(artist):
    a_nospace = re.sub(r'[\s　]', '', artist)
    return (any(n in artist for n in _CHART_MATCH_NAMES)
            or any(n in a_nospace for n in _CHART_MATCH_NOSPACE))
...
def _parse_oricon_daily_chart_page(chart_code, chart_name, date_str):
    """Fetch Oricon DAILY chart page; return (entries, page_ok).

    page_ok is True only when the server returned HTTP 200 (so the caller can
    decide whether to mark this date as "scanned" — a 404/timeout should not
    be marked so it is retried on future runs).
    """
    url = f'https://www.oricon.co.jp/rank/{chart_code}/d/{date_str}/'
    try:
        r = requests.get(url, headers=HEADERS, timeout=14)
        if r.status_code != 200:
            return [], False   # page missing / blocked — do NOT mark as scanned
        if not r.encoding or r.encoding.lower() in ('iso-8859-1', 'latin-1'):
            r.encoding = r.apparent_encoding or 'shift_jis'
        soup = BeautifulSoup(r.text, 'html.parser')
        entries = []
        for sec in soup.select('section.box-rank-entry'):
            ...
            for li in sec.select('ul.list li'):
                ...
                if '発売日' in t:
                    ...
                    label = t
            entries.append({
                'id':          make_id(f'd_{chart_code}_{date_str}_{rank}_{artist}_{title}'),
                'chart_code':  chart_code,
                'chart_name':  chart_name,
                'date':        date_str,
                'period_type': 'daily',
        ...
        return entries, True   # page_ok = True, mark as scanned
    except Exception as e:
        print(f'  [Daily Chart] {url}: {e}')
        return [], False   # exception — do NOT mark as scanned


def fetch_oricon_daily_charts():
    """Scrape last 30 days of Oricon daily charts for M!LK/member appearances."""
    print('Fetching Oricon daily charts...')
    today = datetime.now(JST).replace(tzinfo=None)
    recent_cutoff = (today - timedelta(days=7)).strftime('%Y-%m-%d')

    ex_scanned_raw = load_json(CHART_DAILY_SCANNED_FILE)
    scanned_pairs = {(e['chart_code'], e['date']) for e in ex_scanned_raw
                     if e.get('chart_code') and e.get('date') and e['date'] < recent_cutoff}
    # Also treat existing daily chart entries as scanned (older than 7 days)
    ex_charts_raw = load_json(CHARTS_FILE)
    for e in ex_charts_raw:
        if (e.get('period_type') == 'daily' and e.get('chart_code') and e.get('date')
                and e['date'] < recent_cutoff):
            scanned_pairs.add((e['chart_code'], e['date']))

    daily_dates = [(today - timedelta(days=d)).strftime('%Y-%m-%d') for d in range(62)]
    ...
    for chart_code, chart_name in _DAILY_TARGETS:
        found_code = 0
        for date_str in daily_dates:
            if (chart_code, date_str) in scanned_pairs:
                continue
            entries, page_ok = _parse_oricon_daily_chart_page(chart_code, chart_name, date_str)
            for e in entries:
                if e['id'] not in seen_ids:
                    seen_ids.add(e['id'])
                    all_entries.append(e)
                    found_code += 1
            # Only mark as scanned when the page actually returned HTTP 200
            # (don't mark 404/timeout so those dates are retried on the next run)
            if page_ok:
                newly_scanned.append({'chart_code': chart_code, 'date': date_str})
            time.sleep(0.3)
        if found_code:
            print(f'  [日別 {chart_name}] {found_code} entries')

    return all_entries, newly_scanned


def fetch_billboard_japan_charts():
    """Scrape Billboard Japan weekly charts (SSR — no Playwright needed) for M!LK/member entries.

    URL format: ?a={chart_code}&year=YYYY&month=MM&day=DD
    Each entry is inside <div class="rank-item">; rank in <div class="rank-number">;
    artist via <a href="/artists/detail/...">; title via <h3>.
    """
    BJ_CHARTS = [
        ('hot100',   'BJ Hot 100'),
        ('sales',    'BJ Singles Sales'),
        ('hot_albums','BJ Hot Albums'),
        ('stsongs',  'BJ Streaming Songs'),
    ]

    today = datetime.now(JST).replace(tzinfo=None)
    # Billboard Japan publishes on Wednesday; chart week starts the Monday before.
    # Try last 8 Mondays so we cover ~2 months.
    days_since_monday = today.weekday()
    last_monday = today - timedelta(days=days_since_monday)
    chart_dates = [(last_monday - timedelta(weeks=w)) for w in range(8)]

    ex_bj_scanned = load_json(CHART_BJ_SCANNED_FILE)
    recent_cutoff = (today - timedelta(days=28)).strftime('%Y-%m-%d')
    scanned_pairs = {(e['chart_code'], e['date']) for e in ex_bj_scanned
                     if e.get('chart_code') and e.get('date') and e['date'] < recent_cutoff}

    all_entries, newly_scanned = [], []
    bj_headers = dict(HEADERS, **{'Accept-Language': 'ja,en;q=0.9'})

    for chart_code, chart_name in BJ_CHARTS:
        found_code = 0
        for date_dt in chart_dates:
            date_str = date_dt.strftime('%Y-%m-%d')
            if (chart_code, date_str) in scanned_pairs:
                continue
            url = (f'https://www.billboard-japan.com/charts/detail'
                   f'?a={chart_code}'
                   f'&year={date_dt.year}'
                   f'&month={date_dt.month:02d}'
                   f'&day={date_dt.day:02d}')
            try:
                r = requests.get(url, headers=bj_headers, timeout=15)
                ...
                if not r.encoding or r.encoding.lower() in ('iso-8859-1', 'latin-1'):
                    r.encoding = r.apparent_encoding or 'utf-8'
                soup  = BeautifulSoup(r.text, 'html.parser')
                # HTML structure: <tr class="rank1" ...>, title in <p class="musuc_title">,
                # artist in <p class="artist_name"><a href="/artists/detail/...">
                items = soup.select('tr[class^="rank"]')
                for item in items:
                    # Rank — <span> inside <td class="rank_td"> or <p class="rank">
                    rank_el = item.select_one('td.rank_td span') or item.select_one('p.rank')
                    try:
                        rank = int(re.sub(r'[^\d]', '', rank_el.get_text()))
                    except Exception:
                        continue
                    # Artist
                    artist_el = (item.select_one('p.artist_name a')
                                 or item.select_one('a[href*="/artists/detail/"]'))
                    artist = clean(artist_el.get_text()) if artist_el else ''
                    if not artist or not _chart_match(artist):
                        continue
                    # Streaming chart: only track M!LK entries ranked top 10
                    if chart_code == 'stsongs' and rank > 10:
                        continue
                    # Title — <p class="musuc_title"> (note: their typo, not ours)
                    title_el = item.select_one('p.musuc_title')
                    title = clean(title_el.get_text()) if title_el else ''
                    # Image
                    img_el = item.select_one('img[src*="/scale/"]')
                    img = ''
                    if img_el:
                        img = img_el.get('src', '')
                        if img.startswith('/'):
                            img = 'https://www.billboard-japan.com' + img
                    all_entries.append({
                        'id':           make_id(f'bj_{chart_code}_{date_str}_{rank}_{artist}_{title}'),
                        'chart_code':   f'bj_{chart_code}',
                        'chart_name':   chart_name,
                        'date':         date_str,
                        'period_type':  'weekly',
                        'rank':         rank,
                        ...
                        'label':        '',
                        'release_date': '',
                        'source':       'billboard_japan',
                    })
                    found_code += 1
                newly_scanned.append({'chart_code': chart_code, 'date': date_str})
            except Exception as e:
                print(f'  [Billboard Japan] {chart_code} {date_str}: {e}')
            time.sleep(0.8)
...
def fetch_oricon_charts():
    ...
    Guard runs per chart-code: if charts.json already has data for this Monday
    for a specific chart code, that code is skipped (others still run).
    Fetches pages 1–3 of each chart (≈ top 60 entries) so solo member rankings
    aren't missed.  Also runs member-name Oricon search to catch solo singles that
    appear under slightly different artist strings.
    """
    print('Fetching Oricon charts...')
    today = datetime.now(JST).replace(tzinfo=None)
    # Oricon weekly chart dates are Mondays (chart published Monday morning JST)
    days_since_monday = today.weekday()  # 0 if today is Mon, …, 6 if Sun
    last_monday = today - timedelta(days=days_since_monday)
    this_week_str = last_monday.strftime('%Y-%m-%d')

    # ── Per-(chart_code, date) guard via chart_scanned.json ──────────────────
    # Only skip pairs older than 4 weeks — recent dates are always re-fetched
    # so a chart that wasn't published at 09:00 gets picked up at 12:00/18:00.
    recent_cutoff = (today - timedelta(days=28)).strftime('%Y-%m-%d')
    ex_scanned_raw = load_json(CHART_SCANNED_FILE)  # list of {chart_code, date}
    scanned_pairs = {(e['chart_code'], e['date']) for e in ex_scanned_raw
                     if e.get('chart_code') and e.get('date')
                     and e['date'] < recent_cutoff}
    # Also treat existing real chart entries as "scanned" (old dates only)
    ex_charts_raw = load_json(CHARTS_FILE)
    for e in ex_charts_raw:
        if e.get('chart_code') and e.get('date') and e['date'] < recent_cutoff:
            scanned_pairs.add((e['chart_code'], e['date']))
    ...
    all_entries, seen_ids = [], set()
    newly_scanned = []   # (chart_code, date) pairs scanned this run → saved to CHART_SCANNED_FILE

    # ── Main chart pages (top 60 per chart code) ──────────────────────────────
    for chart_code, chart_name in _ORICON_CHART_TARGETS:
        found_code = 0
        for date_str in chart_dates:
            if (chart_code, date_str) in scanned_pairs:
                continue   # already processed this (code, date) pair
            for page in range(1, 4):   # pages 1-3 ≈ top 60
                entries = _parse_oricon_chart_page(chart_code, chart_name, date_str, page=page)
                if not entries and page > 1:
                    break  # chart page has no more results
                for e in entries:
                    if e['id'] not in seen_ids:
                        seen_ids.add(e['id'])
                        all_entries.append(e)
                        found_code += 1
                time.sleep(0.3 if entries else 0.1)
            # Mark as scanned regardless of whether M!LK was found
            newly_scanned.append({'chart_code': chart_code, 'date': date_str})
        if found_code:
            print(f'  [{chart_name}] {found_code} entries')

    # ── Member solo search: parse Oricon artist pages directly ───────────────
    # Oricon provides artist-specific ranking pages once we know the artist slug.
    # We discover the slug by scraping the artist search page for each member.
    _SOLO_MEMBER_QUERIES = [
        ('吉田仁人', '吉田仁人'),
        ('佐野勇斗', '佐野勇斗'),
        ('山中柔太朗', '山中柔太朗'),
        ('塩﨑太智', '塩崎太智'),   # common alternative kanji
        ('曽野舜太', '曽野舜太'),
    ]
    for canonical, search_q in _SOLO_MEMBER_QUERIES:
        try:
            search_url = f'https://www.oricon.co.jp/search/?q={_url_quote(search_q)}&opt_artist=on'
            sr = requests.get(search_url, headers=HEADERS, timeout=12)
            if sr.status_code != 200:
                continue
            sr.encoding = sr.apparent_encoding or 'utf-8'
            ssoup = BeautifulSoup(sr.text, 'html.parser')
            # Find the first /prof/XXXXX/ link — that's the artist profile
            prof_link = ssoup.select_one('a[href*="/prof/"]')
            if not prof_link:
                continue
            prof_href = prof_link.get('href', '')
            # Artist profile ranking: /prof/XXXXX/ranking/
            m_prof = re.search(r'/prof/(\d+)/', prof_href)
            if not m_prof:
                continue
            artist_id = m_prof.group(1)
            # Scrape the artist ranking page for recent single/digital chart appearances
            rank_url = f'https://www.oricon.co.jp/prof/{artist_id}/ranking/'
            rr = requests.get(rank_url, headers=HEADERS, timeout=12)
            if rr.status_code != 200:
                continue
            rr.encoding = rr.apparent_encoding or 'utf-8'
            rsoup = BeautifulSoup(rr.text, 'html.parser')
            for row in rsoup.select('tr, .ranking-row, li.ranking-item'):
                rank_el = row.select_one('.rank, .num, td:first-child')
                title_el = row.select_one('.title, .work-title, td:nth-child(2)')
                date_el  = row.select_one('.date, .week, time')
                chart_el = row.select_one('.chart, .type')
                if not rank_el or not title_el:
                    continue
                try:
                    rank = int(re.search(r'\d+', rank_el.get_text()).group())
                except Exception:
                    continue
                title = clean(title_el.get_text())[:80]
                if not title:
                    continue
                date_txt = clean(date_el.get_text()) if date_el else ''
                date_str = normalize_date(date_txt) or this_week_str
                if date_str < '2025-01-01':
                    continue
                chart_name_txt = clean(chart_el.get_text()) if chart_el else 'ランキング'
                eid = make_id(f'solo_{canonical}_{title}_{date_str}_{rank}')
                if eid not in seen_ids:
                    seen_ids.add(eid)
                    all_entries.append({
                        'id': eid,
                        'chart_code': 'solo',
                        'chart_name': chart_name_txt or 'ソロ',
                        'date': date_str,
                        'rank': rank,
                        'status': '',
                        'title': title,
            ...
            print(f'  [solo chart] {canonical}: {ex}')
...
def _norm_oricon_url(url):
    """Normalize Oricon article URL to /full/ form. Non-Oricon URLs are returned unchanged."""
    if not url:
        return ''
    if 'oricon.co.jp' not in url:
        return url
    url = url.split('?')[0].split('#')[0].rstrip('/')       # strip query string / fragment first
    url = re.sub(r'/(?:embed(?:/video)?|video|comment)', '', url)   # strip embed/video/comment anywhere
    url = re.sub(r'/p/\d+', '', url)                        # strip /p/N anywhere
    url = re.sub(r'/photo/\d+', '', url)                    # strip /photo/N anywhere
    if '/full' in url:
        url = re.sub(r'/full.*', '/full/', url)             # truncate any sub-path after /full
    else:
        url = url + '/full/'
    return url

def _norm_mdpr_url(url):
    """Convert mdpr.jp preview URL to full-article URL.
    mdpr.jp/{cat}/{id} → mdpr.jp/{cat}/detail/{id}
    The preview page has a 'c-buttonOpenLink' linking to the detail URL;
    we short-circuit directly to avoid needing Playwright or a click.
    """
    if not url or 'mdpr.jp' not in url:
        return url
    # Already a detail URL — leave as-is
    if '/detail/' in url:
        return url
    # Match: /category/DIGITS (optional query string)
    m = re.match(r'(https?://mdpr\.jp/[^/?#]+)/(\d+)([?#].*)?$', url)
    if m:
...
def _fetch_oricon_article(url):
    ...
    try:
        ...
        art['image'] = img
        # Collect all photo sub-page images
        images = [img] if img else []
        seen_imgs = {img} if img else set()
        base = re.sub(r'/full/?$', '', url.rstrip('/'))
        for _i in range(1, 13):
            ...
            try:
                ...
                if _og:
                    _ph = _og.get('content', '').strip()
                    if _ph and _ph not in seen_imgs:
                        seen_imgs.add(_ph)
                        images.append(_ph)
                time.sleep(0.2)
            except Exception:
...
def fetch_oricon():
    ...
    def _is_oricon_photo_article(a):
        """Return True if this is an Oricon photo-gallery article (not a real news article)."""
        title = a.get('title_ja', '') or a.get('title', '') or ''
        return bool(re.match(r'^(画像|写真)[・・]', title))

    gnews_arts = _fetch_rss('Oricon', gnews_url, limit=20)
    for a in gnews_arts:
        u = _norm_oricon_url(a.get('url', ''))
        if u and u not in seen and not _is_oricon_photo_article(a):
            seen.add(u)
            a['url'] = u
            arts.append(a)
...
def fetch_modelpress():
    ...
    seen, arts = set(), []
    # /photo/detail/ は記事本体の画像ギャラリーページ（重複）→ スキップ
    def _is_mdpr_photo(u):
        return '/photo/detail/' in u
    # ① Google News RSS（M!LK 専用クエリ）
    for a in _fetch_rss('Modelpress',
            ...
            limit=15):
        u = a.get('url', '')
        if u and u not in seen and not _is_mdpr_photo(u):
            seen.add(u); arts.append(a)
...
SCHEDULE_PROGRAMS = []  # All program data is fetched dynamically via fetch_sd_milk_programs_pw()

# Auto-fetched schedule cache (populated at runtime by fetch_prog_schedules())
_prog_sched_cache = {}
_prog_dow_cache   = {}  # name → [int] auto-detected DOW list

_DOW_JP = {'月': 0, '火': 1, '水': 2, '木': 3, '金': 4, '土': 5, '日': 6}

def _parse_dow(text):
    """Extract list of DOW indices from broadcast schedule text like '毎週火曜 24:30'."""
    if not text:
        return None
    days = []
    for m in re.finditer(r'([月火水木金土日])曜', text):
        d = _DOW_JP.get(m.group(1))
...
def fetch_prog_schedules():
    """Auto-fetch broadcast schedules for active programs in _dynamic_programs."""
    global _prog_sched_cache, _prog_dow_cache
    # Only fetch TV/ラジオ programs with real URLs
    targets = [
        (p['name'], p['url'])
        for p in _dynamic_programs
        if p.get('type') in ('TV', 'ラジオ')
        and p.get('url', '').startswith('http')
        and 'sd-milk.com/contents/schedule' not in p.get('url', '')
    ]
    if not targets:
        print('Fetching program schedules... (no dynamic programs yet, skipping)')
        return
    print(f'Fetching program schedules for {len(targets)} programs...')
    for name, url in targets:
        sched = _fetch_prog_sched(name, url)
        if sched:
            _prog_sched_cache[name] = sched
            dow = _parse_dow(sched)
            if dow:
                _prog_dow_cache[name] = dow
            print(f'  [{name}] → {sched} (dow={dow})')
        else:
...
def fetch_sd_milk_programs_pw():
    ...
    try:
        _SCHEDULE_URLS = [
            ...
            'https://sd-milk.com/contents/schedule/tv',
            'https://sd-milk.com/contents/schedule/radio',
        ]
        all_soups = []
        with sync_playwright() as p:
            browser = p.chromium.launch(headless=True)
            pw_page = browser.new_page(extra_http_headers={
                'User-Agent': HEADERS['User-Agent'],
                'Accept-Language': 'ja,en;q=0.9',
            })
            for _surl in _SCHEDULE_URLS:
                try:
                    pw_page.goto(_surl, timeout=25000)
                    try:
                        pw_page.wait_for_selector('a, article, li', timeout=12000)
                    except PWTimeout:
                        pass
                    all_soups.append(BeautifulSoup(pw_page.content(), 'html.parser'))
                except Exception as _pe:
                    print(f'  sd-milk schedule fetch error ({_surl}): {_pe}')
            browser.close()

        # Use main schedule page soup as primary, extras supplement
        DATE_RE = re.compile(r'(\d{4})[./年-](\d{1,2})[./月](\d{1,2})')

        def _norm_name(s):
            """Normalize program name for dedup: remove spaces/punctuation, lowercase."""
            s = re.sub(r'[\s　！!【】「」『』\(\)（）・\-_—]', '', s)
            s = re.sub(r'\d{4}', '', s)  # strip years
            return s.lower()

        seen_norms = set()

        # Gather candidates from ALL fetched pages
        candidates = []
        for soup in all_soups:
            # Prefer a[href] and li; skip generic div/article which contain nested elements
            # Process a elements first (cleanest names), then li
            for a in soup.find_all('a', href=True):
                href = a.get('href', '')
                if href.startswith('/') or href.startswith('http'):
                    candidates.append(a)
            for li in soup.find_all('li'):
                # Skip li that directly contains an a already captured
                if not li.find('a', href=True):
                    candidates.append(li)

        for el in candidates:
            text = re.sub(r'\s+', ' ', el.get_text()).strip()
            # Names from a-tags should be short; li can be longer but cap at 120
            max_len = 80 if el.name == 'a' else 120
            ...
            # Name: first meaningful line, max 60 chars
            lines = [l.strip() for l in text.splitlines() if l.strip() and len(l.strip()) > 3]
            name = lines[0][:60] if lines else text[:60]
            name_norm = _norm_name(name)
            if not name_norm:
                continue
            # Dedup: skip if exact norm seen, or if both norms are long enough and one contains the other
            if name_norm in seen_norms:
                continue
            if len(name_norm) >= 6 and any(
                (name_norm in s or s in name_norm) for s in seen_norms if len(s) >= 6
            ):
                continue
            ...
            # Skip placeholder/broken URLs (e.g. /movies/null/null)
            if '/null' in url:
                continue

            # Override keyword-detected status with date-based logic when start_date is known
            if start_date:
                try:
                    from datetime import date as _date_cls
                    today_d = _date_cls.today()
                    sd_d    = _date_cls.fromisoformat(start_date)
                    ed_d    = _date_cls.fromisoformat(end_date) if end_date else None
                    if ed_d and ed_d < today_d:
                        status = 'ended'   # finished airing; caller can skip
                    elif sd_d <= today_d:
                        status = 'active'
                    else:
                        status = 'upcoming'
...
def fetch_setlistfm(include_past=True, past_years=13):
    """setlist.fm から M!LK のライブ日程を取得（過去 + 未来、複数ページ対応）"""
    base_url = 'https://www.setlist.fm/setlists/mlk-53cb6b39.html'
    ...
    def _parse_page(soup):
        ...
        for container in soup.find_all('div', class_='setlist'):
            ...
            link = _abs_link(link_a['href']) if link_a else base_url
            title = f'[LIVE] {venue}' if venue else '[LIVE] M!LK Concert'
            uid = make_id(title[:50] + date_str)
        ...
        return found_any, hit_cutoff

    # Fetch up to 25 pages (each page ~20 events; M!LK debuted 2014 → ~250+ concerts)
    for page in range(1, 26):
        ...
        try:
            ...
            if r.status_code != 200:
                print(f'  setlist.fm page {page} HTTP {r.status_code}')
                break
            r.encoding = 'utf-8'
        except Exception as e:
            print(f'  setlist.fm page {page} error: {e}')
            break
        soup = BeautifulSoup(r.text, 'html.parser')
        found, hit_cutoff = _parse_page(soup)
        if hit_cutoff or not found:
            break
        time.sleep(0.5)
...
def _sdmilk_fetch_body_text(url):
    """Lightweight fetch of a sd-milk.com article page; returns plain text for date extraction.
    Returns '' on failure. Used to find event dates when the title alone doesn't contain them.
    sd-milk.com news detail pages are server-side rendered — requests.get() works without Playwright.
    """
    if not url or 'sd-milk.com' not in url:
        return ''
    try:
        r = requests.get(url, headers=HEADERS, timeout=12)
        if r.status_code != 200:
        ...
        for tag in soup.find_all(['script', 'style', 'nav', 'header', 'footer']):
            tag.decompose()
        return re.sub(r'\s+', ' ', soup.get_text()).strip()[:3000]
    except Exception:
        return ''


def _extract_event_date_from_text(title, summary, pub_date_str):
    """Try to extract the actual event/broadcast date from article text.
    Prefers the nearest future date; falls back to nearest past date within 7 days;
    otherwise returns pub_date_str."""
    ...
    for m in re.finditer(r'(?:(\d{4})年\s*)?(\d{1,2})月(\d{1,2})日', text):
        year  = int(m.group(1)) if m.group(1) else now_jst_date.year
        month = int(m.group(2))
        day   = int(m.group(3))
        try:
            candidate = _dt.date(year, month, day)
            # If year was guessed and candidate is far in past, try next year
            if not m.group(1) and (now_jst_date - candidate).days > 180:
                candidate = _dt.date(year + 1, month, day)
            delta = (candidate - now_jst_date).days
            if 0 <= delta <= 365:
    ...
    if future_candidates:
        return str(min(future_candidates))  # nearest upcoming date
    if past_candidates:
        return str(max(past_candidates))    # most recent past date within 7 days
    return pub_date_str


def fetch_gnews_schedule_events():
    """Search Google News RSS for M!LK event/concert/broadcast announcements → schedule entries."""
    _KW = {'ライブ', 'コンサート', '出演', '公演', 'ツアー', '放送', '特番', 'イベント',
           'fanmeet', 'tour', 'concert', 'live', 'event', 'ファンミ', '冠番', '冠番組',
           'ゴールデン', '生放送', '収録', 'MC', 'パーソナリティ'}
    queries = [
        '"M!LK" (ライブ OR コンサート OR 公演 OR ツアー OR ファンミ)',
        '"M!LK" (出演 OR 放送 OR 特番 OR イベント)',
        '"M!LK" (冠番 OR 冠番組 OR ゴールデン OR 生放送 OR 特番)',
        '("吉田仁人" OR "佐野勇斗" OR "山中柔太朗" OR "塩﨑太智" OR "曽野舜太") (出演 OR 放送 OR ライブ OR 冠番)',
    ]
    events, seen = [], set()
    print('Fetching Google News schedule events...')
    for q in queries:
        rss = ('https://news.google.com/rss/search'
               f'?q={_url_quote(q)}&hl=ja&gl=JP&ceid=JP%3Aja')
        try:
            ...
            for e in feed.entries[:15]:
                ...
                if not title:
                    continue
                if not any(kw in title + summary for kw in _KW):
                    continue
                pub_date = normalize_date(e.get('published', ''))
                if not pub_date or pub_date < '2025-01-01':
                    continue
                # Use actual event date extracted from text, fall back to pub date
                date_str = _extract_event_date_from_text(title, summary, pub_date)
                uid = make_id(title[:50] + date_str)
                if uid in seen:
                    continue
                seen.add(uid)
            ...
            print(f'  [gnews schedule] {ex}')
    print(f'  Google News schedule: {len(events)} events')
    return events


def fetch_sd_milk_official_news():
    """Scrape sd-milk.com/contents/news (static HTML) and return categorized items.

    Returns list of dicts:
      id, date, title, category, url
    Categories: LIVE, MEDIA, RELEASE, TICKET, GOODS, FC, OTHER
    """
    _CATS = ['LIVE EVENT', 'MEDIA', 'RELEASE', 'TICKET', 'GOODS', 'FC']
    _CAT_NORM = {
        'LIVE EVENT': 'LIVE', 'MEDIA': 'MEDIA', 'RELEASE': 'RELEASE',
        'TICKET': 'TICKET', 'GOODS': 'GOODS', 'FC': 'FC',
    }
    items = []
    seen = set()
    print('Fetching sd-milk official news...')
    for page in range(1, 4):
        url = (f'https://sd-milk.com/contents/news/page/{page}'
               if page > 1 else 'https://sd-milk.com/contents/news')
        try:
            r = requests.get(url, headers=HEADERS, timeout=15)
            if r.status_code != 200:
                break
            ...
            for li in soup.select('li'):
                ...
                if not a:
                    continue
                href = a.get('href', '')
                if not re.match(r'^/contents/\d+', href):
                    continue
                text = a.get_text(separator='\n').strip()
                dm = re.search(r'(\d{4})\.\s*(\d{2})\.(\d{2})', text)
                if not dm:
                    continue
                ...
                for c in _CATS:
                    if c in text:
                        ...
                        break
                title = text
                title = re.sub(r'\d{4}\.\s*\d{2}\.\d{2}', '', title)
                for c in _CATS:
            ...
            print(f'  [sd-milk news] page {page} error: {e}')
            break
    print(f'  sd-milk official news: {len(items)} items')
    return items


def _fetch_sd_milk_article_detail(url, default_cat='リリース', soup=None):
    """Fetch one sd-milk.com article and extract structured product info.

    Pass `soup` to skip the HTTP fetch (e.g. when the page was already rendered
    by Playwright and the caller passes the BeautifulSoup object directly).
    """
    try:
        if soup is None:
            r = requests.get(url, headers=HEADERS, timeout=15)
        ...
        # ── Title ──
        title_el = (soup.find('h1', class_=re.compile(r'title|heading|entry', re.I))
        ...
        # ── Published date ──
        pub_date = ''
        for _at in [{'property': 'article:published_time'}, {'name': 'date'},
                    {'itemprop': 'datePublished'}]:
            el = soup.find('meta', _at)
            if el and el.get('content'):
                m = _PUB_DATE_RE.search(el['content'])
                if m:
                    pub_date = f'{m.group(1)}-{m.group(2)}-{m.group(3)}'
        ...
        # ── Release date (from body text: YYYY年M月D日 発売) ──
        body_text = soup.get_text()
        rel_m = re.search(
            r'(\d{4})年\s*(\d{1,2})月\s*(\d{1,2})日\s*[（(（(]?[月火水木金土日]?[）)）)]?\s*発売',
            body_text)
        release_date = ''
        release_label = ''
        if rel_m:
            release_date  = f'{rel_m.group(1)}-{rel_m.group(2).zfill(2)}-{rel_m.group(3).zfill(2)}'
            release_label = f'{rel_m.group(1)}年{rel_m.group(2)}月{rel_m.group(3)}日発売'

        # ── Images ──
        images = []
        for _at in [{'property': 'og:image'}, {'name': 'twitter:image'}]:
            el = soup.find('meta', _at)
            if el and el.get('content', '').strip():
                images.append(_clean_img_url(el['content'].strip()))
                break
        body_el = soup.select_one(
            '.entry-content,.post-content,.article-body,.content-body,.wp-content,article')
        if body_el:
            for img in body_el.find_all('img', src=True):
                src = _clean_img_url(img.get('src', '').strip())
                if src and src not in images and not any(w in src.lower() for w in _SKIP_IMG_WORDS):
                    images.append(src)

        # ── Formats + prices (＜商品形態＞ block) ──
        formats = []
        if body_el:
            raw = body_el.get_text(separator='\n')
            sec_m = re.search(r'[＜<]商品形態[＞>](.*?)(?:\n\s*\n|\Z)', raw, re.DOTALL)
            if sec_m:
                for line in sec_m.group(1).splitlines():
                    line = line.strip()
                    pm = re.search(r'¥[\d,]+(?:\s*\(税込\))?', line)
                    if pm:
                        name = line[:line.index(pm.group())].rstrip('　 \t')
                        formats.append({'name': name or line, 'price': pm.group()})
        # Fallback: scrape lines that contain ¥ and look like format lines
        if not formats:
            for line in body_text.splitlines():
                line = line.strip()
                if '¥' in line and ('盤' in line or 'CD' in line or 'Blu' in line or 'DVD' in line):
                    pm = re.search(r'¥[\d,]+(?:\s*\(税込\))?', line)
                    if pm:
                        name = line[:line.index(pm.group())].rstrip('　 \t')
                        formats.append({'name': name, 'price': pm.group()})

        # ── Purchase period (予約受付期間 / 販売期間) ──
        _DATE_PAT = r'(\d{4})年\s*(\d{1,2})月\s*(\d{1,2})日'
        def _parse_ymd(m):
            return f'{m.group(1)}-{m.group(2).zfill(2)}-{m.group(3).zfill(2)}'
        def _fmt_ymd(m):
        ...
        sale_start = sale_end = ''
        # Try range pattern first: 〇年〇月〇日〜〇年〇月〇日
        range_m = re.search(
            r'(?:予約|販売|受付|注文|購入)[^\n]{0,20}' + _DATE_PAT + r'[^\d]{1,6}' + _DATE_PAT,
            body_text)
        if range_m:
            # First date = start, second date = end
            start_m = re.search(_DATE_PAT, range_m.group())
            parts   = list(re.finditer(_DATE_PAT, range_m.group()))
            if len(parts) >= 2:
                sale_start = _fmt_ymd(parts[0])
                sale_end   = _fmt_ymd(parts[1])
        else:
            # Try individual labels
            for kw_start in ['予約受付開始', '販売開始', '受付開始', '予約開始']:
                sm = re.search(kw_start + r'[^\d]{0,10}' + _DATE_PAT, body_text)
                if sm:
                    pm = re.search(_DATE_PAT, sm.group())
                    if pm:
                        sale_start = _fmt_ymd(pm)
                    break
            for kw_end in ['予約受付終了', '販売終了', '受付終了', '予約締切', '締切']:
                em = re.search(kw_end + r'[^\d]{0,10}' + _DATE_PAT, body_text)
                if em:
                    pm = re.search(_DATE_PAT, em.group())
                    if pm:
                        sale_end = _fmt_ymd(pm)
                    break
        ...
        seen_bl = set()
        STORE_DOMAINS = {
            'jvcmusic', 'universal-music', 'victor-store', 'tower', 'amazon',
            'hmv', 'rakuten', 'mu-mo', 'animate', 'sonymusic', 'kingrecords',
            'avex', 'nipponcolumbia', 'ponycanyon', 'neowing', 'yesasia',
        }
        KW_BUY = {'予約', '購入', '申込', 'こちら', 'SHOP', 'ORDER', 'BUY', 'STORE'}
        for a_tag in soup.find_all('a', href=True):
            href = a_tag['href']
            if not href.startswith('http') or 'sd-milk.com' in href:
                continue
            if href in seen_bl:
                continue
            link_text = clean(a_tag.get_text())
            is_store = any(d in href.lower() for d in STORE_DOMAINS)
            is_buy   = any(k in link_text for k in KW_BUY)
            if is_store or is_buy:
                seen_bl.add(href)
                buy_links.append({'text': link_text or 'ご予約・購入', 'url': href})

        # ── Article body → translate ──
        body_ja = ''
        body_zh = ''
        if body_el:
            paras = []
            for child in body_el.find_all(['p', 'h2', 'h3', 'li']):
                t = re.sub(r'\s+', ' ', child.get_text()).strip()
                if len(t) > 8 and '¥' not in t:
                    paras.append(t)
            body_ja = '\n\n'.join(paras[:12])   # cap at 12 paragraphs
        if not body_ja:
            # br-based fallback
            raw = body_el.get_text(separator='\n').strip() if body_el else ''
            lines = [re.sub(r'\s+', ' ', ln).strip() for ln in raw.splitlines()
                     if len(ln.strip()) > 8 and '¥' not in ln]
            body_ja = '\n\n'.join(lines[:12])
        if body_ja:
            body_zh = translate(body_ja[:1500]) or ''  # translate up to 1500 chars
        ...
        return {
            ...
            'formats':       formats,
            'buy_links':     buy_links,
            'body_ja':       body_ja[:1500],
            'body_zh':       body_zh,
        }
    except Exception as e:
        print(f'  [article detail] {url}: {e}')
        return None


def fetch_sd_milk_release_news(ex_ids=None, max_per_cat=15):
    """Scrape sd-milk.com/contents/news/release and /goods using Playwright (CSR pages)."""
    ex_ids = ex_ids or set()
    CAT_URLS = [
        ('https://sd-milk.com/contents/news/release', 'リリース'),
        ('https://sd-milk.com/contents/news/goods',   'グッズ'),
    ]
    items = []
    seen_urls = set()
    ...
    try:
        ...
        print('  Playwright not installed, skip release news fetch')
        return items
    ...
    try:
        with sync_playwright() as p:
            ...
            for base_url, cat_label in CAT_URLS:
                found = 0
                for page_num in range(1, 4):
                    if found >= max_per_cat:
                        break
                    url = f'{base_url}/page/{page_num}' if page_num > 1 else base_url
                    try:
                        pw_page.goto(url, timeout=25000)
                        try:
                            pw_page.wait_for_selector('a[href]', timeout=12000)
                        except PWTimeout:
                            pass
                        soup = BeautifulSoup(pw_page.content(), 'html.parser')
                        # Extract all article-like links from rendered DOM
                        article_urls = []
                        for a_tag in soup.find_all('a', href=True):
                            href = a_tag['href']
                            # Match /contents/NNNN or /contents/news/.../NNNN
                            if not re.search(r'/contents/[\w/]*\d{4,}', href):
                                continue
                            full_url = ('https://sd-milk.com' + href
                                        if href.startswith('/') else href)
                            if ('sd-milk.com' in full_url and full_url not in seen_urls
                                    and not re.search(r'/news/release/?$|/news/goods/?$|/page/', full_url)):
                                seen_urls.add(full_url)
                                article_urls.append(full_url)
                        if not article_urls:
                            print(f'  [release news] {url}: no article links found in rendered page')
                            break
                        for art_url in article_urls:
                            if found >= max_per_cat:
                                break
                            art_id = make_id(art_url)
                            if art_id in ex_ids:
                                found += 1
                                continue
                            # Navigate to article page with Playwright (CSR — requests won't render JS)
                            try:
                                pw_page.goto(art_url, timeout=25000)
                                try:
                                    pw_page.wait_for_selector('h1,h2,article', timeout=10000)
                                except PWTimeout:
                                    pass
                                art_soup = BeautifulSoup(pw_page.content(), 'html.parser')
                            except Exception as _e:
                                print(f'  [release news] article load failed: {_e}')
                                art_soup = None
                            item = _fetch_sd_milk_article_detail(art_url, cat_label, soup=art_soup)
                            if item:
                                items.append(item)
                                found += 1
                            time.sleep(0.5)
                    except Exception as e:
                        print(f'  [release news] {url}: {e}')
                        break
                    time.sleep(0.3)
            browser.close()
    except Exception as e:
        print(f'  release news Playwright error: {e}')

    print(f'  sd-milk release/goods articles: {len(items)} new')
    return items


def fetch_schedule(official_news=None):
    ...
    # ── Step 0: Use pre-fetched official news (LIVE/MEDIA) as schedule events ──
    _SCHED_CATS = {'LIVE', 'MEDIA'}
    try:
        _off_news = official_news or fetch_sd_milk_official_news()
        for item in _off_news:
            if item.get('category') not in _SCHED_CATS:
                continue
            title_ja = item.get('title', '')
            pub_date = item.get('date', '')
            # Always fetch the article body — LIVE/MEDIA titles almost never contain the event
            # date; it's in the body (e.g. "8月3日(月)11:55〜放送").
            body_text = _sdmilk_fetch_body_text(item.get('url', ''))
            date_str = _extract_event_date_from_text(title_ja, body_text, pub_date)
            time.sleep(0.3)
            events.append({
                'id':       item['id'],
                'date':     date_str,
        ...
        if events:
            print(f'  Step 0 (official news HTML): {len(events)} schedule events')
    except Exception as _e0:
        print(f'  Step 0 error: {_e0}')
            ...
            if nd:
                ...
                for a in nd:
                    ...
                    if any(kw in combined for kw in SCHED_KW) and a.get('date'):
                        pub_date = normalize_date(a['date'])
                        if pub_date[:4] < '2025':
                            continue
                        title_ja   = a.get('title_ja', '')
                        summary_ja = a.get('summary_ja', '')
                        href = a.get('url', '')
                        # Always fetch body — event dates are in the article body, not titles
                        body_text = _sdmilk_fetch_body_text(href) if href and 'sd-milk.com' in href else ''
                        text_for_date = summary_ja + ' ' + body_text
                        date_str = _extract_event_date_from_text(title_ja, text_for_date, pub_date)
                        time.sleep(0.3)
                        news_arts.append({
                            'id':       make_id(title_ja[:50] + date_str),
                            'date':     date_str,
                            'title':    title_ja[:120],
                            'details':  '',
                            'title_zh': '',
...
@media(max-width:600px){
  .page-nav{justify-content:flex-start;-webkit-mask-image:linear-gradient(to right,#000 75%,transparent 100%);mask-image:linear-gradient(to right,#000 75%,transparent 100%);}
}
...
.cal-today-btn{font-size:.68rem;font-weight:700;letter-spacing:.02em;}
...
#back-to-today{position:fixed;bottom:22px;right:22px;z-index:999;
  background:var(--blue);color:#fff;border:none;border-radius:20px;
  padding:8px 16px;font-size:.78rem;font-weight:600;letter-spacing:.05em;
  cursor:pointer;box-shadow:0 2px 10px rgba(42,91,215,.35);
  transition:background .15s;}
#back-to-today:hover{background:var(--blue2);}
...
NAV_CHART_BJ  = _build_nav('chart_bj.html')
...
<div class="main-wrap">
  <aside class="calendar-wrap">
    <div class="cal-nav">
      ...
      <button class="cal-arrow cal-today-btn" id="cal-today" title="回到今日">今</button>
    </div>
  ...
  <button id="back-to-today">↑ 今日</button>
</div>
<footer>
...
document.getElementById('cal-today').addEventListener('click',function(){
  ...
  selected=datesWithNews.has(todayStr)?todayStr:null;
  var ys=document.getElementById('cal-year-sel');if(ys)ys.value=calYear;
  var pk=document.getElementById('cal-picker');if(pk)pk.classList.remove('open');
  renderMonthRow();renderCalendar();renderArticles();
});
const SC=__SC__;
...
function renderArticles(){
  ...
  if(selected){
    ...
    sd.forEach(function(date){
      ...
      if(date===todayStr){grp.id='today-group';}
...
renderMonthRow();renderCalendar();renderArticles();
(function(){
  var btn=document.getElementById('back-to-today');
  if(!btn)return;
  btn.addEventListener('click',function(){
    var todayGrp=document.getElementById('today-group');
    if(todayGrp)todayGrp.scrollIntoView({behavior:'smooth',block:'start'});
  });
})();
</script>
</body>
...
/* ── schedule calendar ──────────────────────────────────────────────────── */
.sc-cal-layout{display:grid;grid-template-columns:260px 1fr;gap:14px;align-items:start;overflow:hidden;}
...
/* right panel */
.sc-events-col{background:var(--card);border:1px solid var(--border);border-radius:14px;padding:14px;box-shadow:var(--shadow);min-height:240px;min-width:0;overflow:hidden;}
...
.sc-upcoming-title{font-size:.82rem;font-weight:600;flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}
...
/* past accordion (pre-current-year) */
.past-acc{margin-bottom:12px;border:1px solid var(--border);border-radius:10px;overflow:hidden;}
.past-acc summary{display:flex;align-items:center;gap:10px;padding:10px 14px;cursor:pointer;font-size:.78rem;font-weight:700;color:var(--text);background:var(--card);list-style:none;user-select:none;}
.past-acc summary::-webkit-details-marker{display:none;}
.past-acc summary::before{content:'▶';font-size:.6rem;color:var(--muted);transition:transform .2s;}
.past-acc[open]>summary::before{transform:rotate(90deg);}
.past-acc summary .past-acc-year{color:var(--blue2);font-weight:800;letter-spacing:.1em;}
.past-acc summary .past-acc-count{font-size:.65rem;color:var(--muted);font-weight:400;margin-left:auto;}
.past-acc .past-list{padding:10px 12px 12px;}
/* past live accordion inside ライブ・イベント */
.past-live-acc{width:100%;}
.past-live-acc>summary{display:flex;align-items:center;gap:8px;padding:10px 0;cursor:pointer;font-size:.72rem;letter-spacing:.25em;text-transform:uppercase;font-weight:700;color:var(--blue2);list-style:none;user-select:none;}
.past-live-acc>summary::-webkit-details-marker{display:none;}
.past-live-acc>summary::before{content:'▶';font-size:.6rem;color:var(--muted);transition:transform .2s;letter-spacing:0;}
.past-live-acc[open]>summary::before{transform:rotate(90deg);}
/* ── official news categories ──────────────────────────────────────────── */
.offnews-tabs{display:flex;flex-wrap:wrap;gap:8px;margin-bottom:16px;}
.offnews-tab{padding:5px 14px;border-radius:20px;font-size:.72rem;font-weight:700;cursor:pointer;border:none;background:var(--border);color:var(--text);transition:background .15s;}
.offnews-tab.active{background:var(--blue3);color:#fff;}
.offnews-list{display:flex;flex-direction:column;gap:6px;}
.offnews-item{display:flex;align-items:center;gap:10px;padding:9px 12px;background:var(--card);border:1px solid var(--border);border-radius:10px;text-decoration:none;color:inherit;transition:background .15s;}
.offnews-item:hover{background:#f0f7ff;}
.offnews-item[style*="display:none"]{display:none!important;}
.offnews-cat{font-size:.6rem;font-weight:700;color:#fff;padding:2px 9px;border-radius:8px;flex-shrink:0;min-width:48px;text-align:center;}
.offnews-date{font-size:.65rem;color:var(--muted);white-space:nowrap;font-variant-numeric:tabular-nums;}
.offnews-title{font-size:.83rem;font-weight:600;flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}
/* ── dynamic programs ───────────────────────────────────────────────────── */
...
<header class="site-header">
  ...
  <nav class="page-nav">__NAV__</nav>
  <p class="update-time" id="sched-update-time"></p>
</header>
<div class="sched-wrap">

  <div class="sched-section">
    <p class="sched-section-title" id="today-heading">&#128197; 今日の行程</p>
    __TODAY_SCHED__
  </div>
  ...
  <div class="sched-section">
    <p class="sched-section-title">&#128247; 放送中</p>
    __PROG_ONAIR__
  </div>
...
<script>
// Set update time + today heading dynamically (consistent with TAB1)
(function(){
  var upd=document.getElementById('sched-update-time');
  if(upd)upd.textContent='最後更新：__UPDATED_AT__ JST';
  var d=new Date();
  var y=d.getFullYear();
  var mo=('0'+(d.getMonth()+1)).slice(-2);
  var dy=('0'+d.getDate()).slice(-2);
  var dows=['日','月','火','水','木','金','土'];
  var h=document.getElementById('today-heading');
  if(h)h.textContent='📅 '+y+'-'+mo+'-'+dy+'（'+dows[d.getDay()]+'） 今日の行程';
})();
...
.goods-card,.release-card{background:var(--card);border:1px solid var(--border);border-radius:14px;display:flex;flex-direction:column;box-shadow:var(--shadow);transition:transform .2s,box-shadow .2s,border-color .2s;text-decoration:none;color:inherit;cursor:pointer;}
...
/* ── Release compact cards (外層小卡) ── */
.release-rich-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(160px,1fr));gap:14px;}
.rr-card{background:var(--card);border:1px solid var(--border);border-radius:14px;display:flex;flex-direction:column;box-shadow:var(--shadow);cursor:pointer;transition:transform .2s,box-shadow .2s,border-color .2s;overflow:hidden;}
.rr-card:hover{transform:translateY(-3px);box-shadow:0 8px 24px rgba(30,60,120,.18);border-color:#90b8f0;}
.rr-img-wrap{overflow:hidden;aspect-ratio:1/1;background:var(--surface);}
.rr-img-wrap img{width:100%;height:100%;object-fit:cover;display:block;transition:transform .3s;}
.rr-card:hover .rr-img-wrap img{transform:scale(1.04);}
.rr-img-none{aspect-ratio:1/1;display:flex;align-items:center;justify-content:center;font-size:2.5rem;background:var(--surface);color:var(--border);}
.rr-body{padding:10px;display:flex;flex-direction:column;gap:4px;flex:1;}
.rr-date-chip{display:inline-block;font-size:.58rem;font-weight:700;color:#fff;background:#c62828;padding:2px 7px;border-radius:6px;align-self:flex-start;letter-spacing:.03em;}
.rr-date-chip.goods{background:#0277bd;}
.rr-title-zh{font-size:.82rem;font-weight:800;color:var(--blue);line-height:1.4;}
.rr-title{font-size:.68rem;color:var(--muted);line-height:1.4;}
.rr-sale-period{font-size:.6rem;color:var(--muted);font-variant-numeric:tabular-nums;}
.rr-open-hint{font-size:.62rem;color:var(--blue);margin-top:auto;padding-top:6px;font-weight:600;}
/* ── Release modal (點開詳細) ── */
#rr-overlay{display:none;position:fixed;inset:0;background:rgba(0,0,0,.6);z-index:2000;align-items:center;justify-content:center;padding:16px;}
#rr-overlay.open{display:flex;}
#rr-modal{background:var(--card);border-radius:18px;max-width:680px;width:100%;max-height:90vh;overflow-y:auto;box-shadow:0 20px 60px rgba(0,0,0,.4);position:relative;}
.rr-modal-close{position:absolute;top:12px;right:14px;background:none;border:none;font-size:1.4rem;cursor:pointer;color:var(--muted);line-height:1;z-index:10;}
.rr-modal-close:hover{color:var(--text);}
.rr-modal-gallery{display:flex;gap:8px;overflow-x:auto;padding:16px 16px 0;scrollbar-width:thin;}
.rr-modal-gallery img{height:200px;width:auto;border-radius:10px;object-fit:cover;flex-shrink:0;}
.rr-modal-gallery:empty{display:none;}
.rr-modal-body{padding:18px 20px 24px;}
.rr-modal-chip{display:inline-block;font-size:.62rem;font-weight:700;color:#fff;background:#c62828;padding:3px 10px;border-radius:8px;margin-bottom:8px;}
.rr-modal-chip.goods{background:#0277bd;}
.rr-modal-title-zh{font-size:1.08rem;font-weight:800;color:var(--blue);line-height:1.4;margin-bottom:2px;}
.rr-modal-title{font-size:.8rem;color:var(--muted);margin-bottom:12px;}
.rr-modal-formats{border:1px solid var(--border);border-radius:10px;overflow:hidden;margin-bottom:14px;}
.rr-modal-fmt-row{display:flex;justify-content:space-between;align-items:center;padding:8px 14px;font-size:.78rem;border-bottom:1px solid var(--border);}
.rr-modal-fmt-row:last-child{border-bottom:none;}
.rr-modal-fmt-row:nth-child(even){background:var(--surface);}
.rr-modal-fmt-name{color:var(--text);flex:1;}
.rr-modal-fmt-price{color:#c62828;font-weight:700;font-variant-numeric:tabular-nums;}
.rr-modal-buy-row{display:flex;flex-wrap:wrap;gap:8px;margin-bottom:12px;}
.rr-modal-buy-btn{font-size:.75rem;font-weight:700;color:#fff;background:var(--blue);padding:8px 16px;border-radius:14px;text-decoration:none;transition:opacity .2s;}
.rr-modal-buy-btn:hover{opacity:.8;}
.rr-modal-src{font-size:.65rem;color:var(--muted);text-decoration:none;}
.rr-modal-src:hover{color:var(--blue);}
.rr-modal-sale{font-size:.78rem;color:var(--blue);font-weight:600;margin-bottom:10px;}
.rr-modal-members{display:flex;flex-wrap:wrap;gap:6px;margin-bottom:12px;align-items:center;}
.rr-modal-members-label{font-size:.65rem;font-weight:700;color:var(--muted);letter-spacing:.06em;margin-right:4px;}
.rr-modal-member-tag{font-size:.72rem;font-weight:600;background:var(--surface);border:1px solid var(--border);padding:3px 9px;border-radius:8px;color:var(--text);}
.rr-modal-body-zh{font-size:.82rem;line-height:1.7;color:var(--text);border-top:1px solid var(--border);padding-top:12px;margin-bottom:12px;}
.rr-modal-body-zh p{margin:0 0 .6em 0;}
@media(max-width:600px){
  #rr-overlay{padding:0;align-items:flex-end;}
  #rr-modal{border-radius:20px 20px 0 0;max-height:88vh;width:100%;max-width:100%;}
  .rr-modal-close{position:sticky;top:0;display:block;width:100%;text-align:right;padding:12px 16px;background:var(--card);border-radius:20px 20px 0 0;border-bottom:1px solid var(--border);box-sizing:border-box;font-size:1.2rem;}
  .rr-modal-gallery img{height:140px;}
  .rr-modal-body{padding:14px 16px 24px;}
  .rr-modal-fmt-row{padding:10px 12px;font-size:.82rem;}
  .rr-modal-buy-btn{padding:10px 16px;font-size:.8rem;}
}
</style>
</head>
<body>
...
<div id="rr-overlay" onclick="if(event.target===this)closeRrModal()">
  <div id="rr-modal">
    <button class="rr-modal-close" onclick="closeRrModal()">&#10005;</button>
    <div class="rr-modal-gallery" id="rr-modal-gallery"></div>
    <div class="rr-modal-body" id="rr-modal-body"></div>
  </div>
</div>
<script>
function openRrModal(id){
  var d=_rrData[id];if(!d)return;
  var ov=document.getElementById('rr-overlay');
  var gal=document.getElementById('rr-modal-gallery');
  var body=document.getElementById('rr-modal-body');
  // Gallery
  gal.innerHTML='';
  ...
  // Body
  var isGoods=d.category==='グッズ';
  var chipCls='rr-modal-chip'+(isGoods?' goods':'');
  var chipTxt=isGoods?'&#128722; グッズ':'&#127926; '+d.release_label;
  var fmts='';
  if(d.formats&&d.formats.length){
    fmts='<div class="rr-modal-formats">'+d.formats.map(function(f){
      return '<div class="rr-modal-fmt-row"><span class="rr-modal-fmt-name">'+esc(f.name)+'</span><span class="rr-modal-fmt-price">'+esc(f.price)+'</span></div>';
    }).join('')+'</div>';
  }
  var buys='';
  if(d.buy_links&&d.buy_links.length){
    buys='<div class="rr-modal-buy-row">'+d.buy_links.slice(0,5).map(function(b){
      return '<a class="rr-modal-buy-btn" href="'+esc(b.url)+'" target="_blank" rel="noopener">'+(b.text?esc(b.text.slice(0,20)):'予約・購入')+' &#8599;</a>';
    }).join('')+'</div>';
  }
  var membersHtml='';
  if(d.members&&d.members.length){
    membersHtml='<div class="rr-modal-members"><span class="rr-modal-members-label">メンバー</span>'
      +d.members.map(function(m){return '<span class="rr-modal-member-tag">'+esc(m)+'</span>';}).join('')
      +'</div>';
  }
  var bodyHtml='';
  if(d.body_zh){
    var paras=d.body_zh.split('\\n\\n').filter(function(p){return p.trim().length>0;});
    bodyHtml='<div class="rr-modal-body-zh">'
      +paras.map(function(p){return '<p>'+esc(p.trim())+'</p>';}).join('')
      +'</div>';
  }
  var saleHtml='';
  if(d.sale_start||d.sale_end){
    var _saleTxt;
    if(d.sale_start&&d.sale_end)_saleTxt=esc(d.sale_start)+' 〜 '+esc(d.sale_end);
    else if(d.sale_start)_saleTxt=esc(d.sale_start)+' 〜';
    else _saleTxt='〜 '+esc(d.sale_end);
    saleHtml='<p class="rr-modal-sale">&#128197; '+_saleTxt+'</p>';
  }
  var titleHtml=d.title?'<p class="rr-modal-title">'+esc(d.title)+'</p>':'';
  body.innerHTML='<span class="'+chipCls+'">'+chipTxt+'</span>'
    +(d.title_zh?'<p class="rr-modal-title-zh">'+esc(d.title_zh)+'</p>':'')
    +titleHtml
    +membersHtml
    +saleHtml
    +fmts
    +bodyHtml
    +buys
    +'<a class="rr-modal-src" href="'+esc(d.url)+'" target="_blank" rel="noopener">公式情報ページ &#8594;</a>';
...
function esc(s){return String(s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;');}
document.addEventListener('keydown',function(e){if(e.key==='Escape')closeRrModal();});
</script>
</body>
</html>'''
...
def _release_rich_card_html(r, esc):
    """Compact outer card (image + title + chip). Full details open in modal via JS."""
    import json as _json, hashlib as _hl
    _raw_id = r.get('id', '')
    if not _raw_id:
        _raw_id = 'r' + _hl.md5((r.get('url', '') or r.get('title', '')).encode('utf-8', errors='replace')).hexdigest()[:10]
    rid        = esc(_raw_id)
    img        = r.get('image', '')
    title      = esc(r.get('title', ''))
    zh         = esc(r.get('title_zh', ''))
    rlbl       = esc(r.get('release_label', '') or r.get('date', ''))
    cat        = r.get('category', 'リリース')
    sale_start = r.get('sale_start', '')
    sale_end   = r.get('sale_end', '')

    chip_cls = 'rr-date-chip' + (' goods' if cat == 'グッズ' else '')
    chip     = f'<span class="{chip_cls}">{"&#128722; グッズ" if cat == "グッズ" else "&#127926; " + rlbl}</span>' if rlbl else ''

    img_block = (
        f'<div class="rr-img-wrap"><img src="{esc(img)}" alt="" loading="lazy" '
        f'onerror="this.parentElement.innerHTML=\'&#127926;\'"></div>'
        if img else '<div class="rr-img-none">&#127926;</div>'
    )

    # Purchase period row shown on card
    if sale_start and sale_end:
        sale_row = f'<p class="rr-sale-period">&#128197; {esc(sale_start)} 〜 {esc(sale_end)}</p>'
    elif sale_start:
        sale_row = f'<p class="rr-sale-period">&#128197; {esc(sale_start)} 〜</p>'
    else:
        sale_row = ''

    # Serialize release data for JS modal (only safe fields)
    _members = r.get('members') or (
        ['吉田仁人', '佐野勇斗', '山中柔太朗', '塩﨑太智', '曽野舜太']
        if cat != 'グッズ' else []
    )
    js_data = _json.dumps({
        'id':            r.get('id', ''),
        ...
        'release_label': r.get('release_label', '') or r.get('date', ''),
        'sale_start':    sale_start,
        'sale_end':      sale_end,
        'url':           r.get('url', '#'),
        'images':        r.get('images', [])[:12],
        'formats':       r.get('formats', [])[:10],
        'buy_links':     r.get('buy_links', [])[:5],
        'members':       _members,
        'body_zh':       r.get('body_zh', ''),
    }, ensure_ascii=False).replace('</', '<\\/').replace('\u2028', '\\u2028').replace('\u2029', '\\u2029')

    return (
        f'<div class="rr-card" onclick="openRrModal(\'{rid}\')" role="button" tabindex="0" '
        f'onkeydown="if(event.key===\'Enter\')openRrModal(\'{rid}\')">'
        f'<script>_rrData["{rid}"]={js_data};</script>'
        f'{img_block}'
        f'<div class="rr-body">'
        + (chip if chip else '') +
        (f'<p class="rr-title-zh">{zh}</p>' if zh else '') +
        f'<p class="rr-title">{title}</p>'
        + sale_row +
        f'<p class="rr-open-hint">詳細を見る →</p>'
        f'</div></div>'
    )
...
def _goods_card_html(g, esc):
    ...
    cat_raw  = g.get('category', g.get('source', ''))
    cat      = esc(cat_raw)
    zh_raw   = g.get('title_zh', '')
    ja_raw   = g.get('title', '')
    price_raw = g.get('price', '')
    date_raw  = g.get('date', '')
    raw_url  = (g.get('url') or '').strip()
    has_url  = raw_url.startswith('http')
    img      = g.get('image', '')

    # Stable ID for modal lookup
    gid = 'g' + _hl.md5((raw_url or ja_raw).encode('utf-8', errors='replace')).hexdigest()[:10]

    # Data injected into global _rrData so openRrModal can display it
    js_data = _json.dumps({
        'category':      'グッズ',
        'title':         ja_raw,
        'title_zh':      zh_raw,
        'release_label': date_raw,
        'sale_start':    date_raw,
        'sale_end':      '',
        'url':           raw_url if has_url else '#',
        'images':        [img] if img else [],
        'formats':       [{'name': '価格', 'price': price_raw}] if price_raw else [],
        'buy_links':     [{'url': raw_url, 'text': '詳細・購入'}] if has_url else [],
        'members':       [],
    ...
    inner = (
        f'<script>_rrData["{gid}"]={js_data};</script>'
        f'<div class="goods-body">'
        f'<span class="cat-badge" style="background:{color}">{cat}</span>'
        + (f'<p class="goods-title-zh">{esc(zh_raw)}</p>' if zh_raw else '') +
        f'<p class="goods-title">{esc(ja_raw)}</p>'
        + (f'<p class="goods-price">{esc(price_raw)}</p>' if price_raw else '')
        + (f'<p class="goods-date">{esc(date_raw)}</p>' if date_raw else '')
        + '<span class="goods-buy">詳細を見る &rarr;</span>'
        + '</div>'
    )
    return (f'<div class="goods-card" onclick="openRrModal(\'{gid}\')" '
            f'role="button" tabindex="0" style="cursor:pointer">{img_block}{inner}</div>')


def generate_goods_html(goods, updated_at, releases=None):
    def esc(s):
    ...
    # ── Section 0a: 公式リリース情報 ─────────────────────────────────────────
    rel_only  = sorted([r for r in releases if r.get('category') == 'リリース'],
                       key=lambda x: x.get('date', ''), reverse=True)
    if rel_only:
        cards = ''.join(_release_rich_card_html(r, esc) for r in rel_only)
        sections.append(
            f'<div class="goods-section">'
            f'<p class="section-label">&#127926; リリース情報'
            f'<span class="cnt">({len(rel_only)})</span></p>'
            f'<div class="release-rich-grid">{cards}</div>'
            f'</div>'
        )

    # ── Section 0b: 公式グッズ情報 ───────────────────────────────────────────
    goods_news = sorted([r for r in releases if r.get('category') == 'グッズ'],
                        key=lambda x: x.get('date', ''), reverse=True)
    if goods_news:
        cards = ''.join(_release_rich_card_html(r, esc) for r in goods_news)
        sections.append(
            f'<div class="goods-section">'
            f'<p class="section-label">&#128722; グッズ情報'
            f'<span class="cnt">({len(goods_news)})</span></p>'
            f'<div class="release-rich-grid">{cards}</div>'
            f'</div>'
        )
...
@media(max-width:600px){
  ...
  .hist-rank{width:22px;height:22px;font-size:.65rem;}
  .hist-info{min-width:calc(100% - 36px);}
  .hist-meta{flex-basis:100%;padding-left:28px;margin-top:-4px;}
  .hist-title{white-space:normal;font-size:.8rem;}
}
.chart-day-label{color:#059669;border-left-color:#059669;}
.chart-daily-badge{font-size:.56rem;font-weight:800;color:#fff;background:#059669;padding:2px 6px;border-radius:5px;}
</style>
</head>
<body>
<header class="site-header">
...
<div class="sched-wrap" style="max-width:1060px;margin:0 auto;padding:20px 16px 72px;">
  <div class="chart-hero">
    <p class="chart-hero-title">__HERO_TITLE__</p>
...
__FILTER_BUTTONS__
  ...
  <!-- 直近30日 日別チャート -->
  <div id="chart-daily">__DAILY_BODY__</div>
...
_ORICON_FILTER_BUTTONS = '''\
    ...
    <button class="cf-btn" data-code="ep">EP</button>
    <button class="cf-btn" data-code="dis">デジタルシングル</button>
    <button class="cf-btn" data-code="st">ストリーミング</button>'''

_BJ_FILTER_BUTTONS = '''\
    <button class="cf-btn active" data-code="all">すべて</button>
    <button class="cf-btn" data-code="bj_hot100">Hot 100</button>
    <button class="cf-btn" data-code="bj_sales">Singles Sales</button>
    <button class="cf-btn" data-code="bj_hot_albums">Hot Albums</button>
    <button class="cf-btn" data-code="bj_stsongs">Streaming</button>'''

_ORICON_ATTRIBUTION = '<p style="font-size:.68rem;color:var(--muted);margin-top:28px;">データ出典：<a href="https://www.oricon.co.jp/rank/" target="_blank" rel="noopener" style="color:var(--blue)">Oricon</a></p>'
_BJ_ATTRIBUTION     = '<p style="font-size:.68rem;color:var(--muted);margin-top:28px;">データ出典：<a href="https://www.billboard-japan.com/charts/" target="_blank" rel="noopener" style="color:var(--blue)">Billboard Japan</a></p>'


def generate_chart_html(charts, updated_at, source='oricon'):
    ...
    is_bj = (source == 'bj')
    if is_bj:
        charts   = [c for c in charts if c.get('source') == 'billboard_japan']
        nav_html = NAV_CHART_BJ
    else:
        charts   = [c for c in charts if c.get('source') != 'billboard_japan']
        nav_html = NAV_CHART
    ...
    daily_charts  = [] if is_bj else [c for c in charts if c.get('period_type') == 'daily']
    weekly_charts = [c for c in charts if c.get('period_type', 'weekly') == 'weekly']

    recent   = [c for c in weekly_charts if c['date'] >= year_start]   # 今年 週別
    historic = [c for c in weekly_charts if c['date'] <  year_start]   # 去年以前 週別
    ...
    for date in sorted(by_date.keys(), reverse=True):
        ...
        for e in entries:
            ...
            member_tag  = (f'<span class="chart-member-tag">👤 {esc(e["member"])}</span>'
                           if e.get('member', 'M!LK') != 'M!LK' else '')
            href = e.get('url','') or (
                f'https://www.billboard-japan.com/charts/detail?a={e["chart_code"][3:]}'
                if e.get("source") == "billboard_japan"
                else f'https://www.oricon.co.jp/rank/{e["chart_code"]}/w/{e["date"]}/'
            )
            cards.append(
    ...
    if recent_parts:
        recent_body = (
            f'<p class="history-section-title" style="margin-bottom:16px">📊 {this_year}年 チャートイン記録</p>'
            + ''.join(recent_parts)
        )
    else:
        recent_body = f'<div class="chart-empty">{this_year}年のチャートイン記録はまだありません。</div>'
    ...
    if historic:
        ...
        for year in sorted(by_ym.keys(), reverse=True):
            ...
            # All previous years collapsed by default
            year_groups.append(
                ...
                f'</div>'
                f'<div class="year-body">{"".join(month_htmls)}</div>'
                f'</div>'
            )

        history_section = (
            f'<div class="history-wrap" id="chart-history">'
            f'<p class="history-section-title">📂 {this_year}年以前 — クリックで展開</p>'
            + ''.join(year_groups) +
            f'</div>'
        )

    # ── Daily chart section (last 30 days) ──────────────────────────
    daily_by_date = defaultdict(list)
    for c in sorted(daily_charts, key=lambda c: (c['date'], c['chart_code'], c['rank'])):
        daily_by_date[c['date']].append(c)

    daily_parts = []
    for date in sorted(daily_by_date.keys(), reverse=True):
        entries = daily_by_date[date]
        cards = []
        for e in entries:
            img_html = (f'<img class="chart-card-img" src="{esc(e["image"])}" alt="{esc(e["title"])}" loading="lazy">'
                        if e.get('image') else '')
            member_tag = (f'<span class="chart-member-tag">👤 {esc(e["member"])}</span>'
                          if e.get('member', 'M!LK') != 'M!LK' else '')
            href = e.get('url', '') or f'https://www.oricon.co.jp/rank/{e["chart_code"]}/d/{e["date"]}/'
            cards.append(
                f'<a class="chart-card" href="{esc(href)}" target="_blank" rel="noopener"'
                f' data-code="{esc(e["chart_code"])}">'
                + img_html +
                ...
                + rank_badge_span(e['rank']) +
                f'<span class="chart-type-tag">{esc(e["chart_name"])}</span>'
                f'<span class="chart-daily-badge">日別</span>'
                + member_tag +
                f'</div>'
                f'<p class="chart-card-title">{esc(e["title"])}</p>'
                f'<p class="chart-card-artist">{esc(e["artist"])}</p>'
        ...
        daily_parts.append(
            f'<div class="chart-week-group">'
            f'<p class="chart-week-label chart-day-label">📅 {date}</p>'
            f'<div class="chart-grid">{"".join(cards)}</div>'
            f'</div>'
        )
    ...
    if daily_parts:
        daily_body = (
            f'<p class="history-section-title" style="margin-bottom:16px;color:#059669;border-left-color:#059669">'
            f'📊 日別チャートイン記録</p>'
            + ''.join(daily_parts)
        )
    else:
    ...
    weekly_count = len(weekly_charts)
    daily_count  = len(daily_charts)
    total_count  = f'{weekly_count + daily_count} 件'

    hero_title   = 'BILLBOARD JAPAN' if is_bj else 'ORICON CHART'
    filter_btns  = _BJ_FILTER_BUTTONS if is_bj else _ORICON_FILTER_BUTTONS
    attribution  = _BJ_ATTRIBUTION if is_bj else _ORICON_ATTRIBUTION
...
:root {
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .res-tok-desc { color: rgba(255,255,255,0.4); font-size: 11px; }
  ...
  .tokuten-imgs { display: flex; gap: 8px; flex-wrap: wrap; margin-top: 10px; }
  .tokuten-imgs img { height: 140px; width: auto; border-radius: 8px; object-fit: cover; border: 1px solid var(--border); cursor: pointer; transition: opacity .2s; }
  .tokuten-imgs img:hover { opacity: .82; }
  /* ─── LIGHTBOX ─── */
  #alb-lb { display:none; position:fixed; inset:0; background:rgba(0,0,0,.88); z-index:9000; align-items:center; justify-content:center; cursor:zoom-out; }
  #alb-lb.open { display:flex; }
  #alb-lb img { max-width:92vw; max-height:90vh; border-radius:10px; object-fit:contain; box-shadow:0 8px 48px rgba(0,0,0,.7); }
  #alb-lb-close { position:fixed; top:16px; right:20px; background:none; border:none; color:#fff; font-size:2rem; cursor:pointer; line-height:1; z-index:9001; opacity:.7; }
  #alb-lb-close:hover { opacity:1; }
  .store-chip {
...
<header class="hero">
  ...
  <h1 class="hero-title"><em>LOVE ENG!NE</em></h1>
  <p class="hero-subtitle">1st Mini Album · 2026.09.16</p>
  <div class="hero-meta">
...
<section class="section">
  ...
  <div class="editions-top">
    <div class="plain-wrap">
      <div class="card">
        <div class="card-cover"><img src="editions_img_0.jpg" alt="通常盤 ジャケット" loading="lazy" onerror="this.style.display='none'"></div>
    ...
    <div class="holo-wrap">
      <div class="card">
        <div class="card-cover"><img src="editions_img_1.jpg" alt="初回限定盤 ジャケット" loading="lazy" onerror="this.style.display='none'"></div>
  ...
  <div class="editions-solo">
    <div class="holo-wrap"><div class="card">
      <div class="card-cover"><img src="editions_img_2.jpg" alt="佐野勇斗盤 ジャケット" loading="lazy" onerror="this.style.display='none'"></div>
    ...
    <div class="holo-wrap"><div class="card">
      <div class="card-cover"><img src="editions_img_3.jpg" alt="塩﨑太智盤 ジャケット" loading="lazy" onerror="this.style.display='none'"></div>
      <span class="card-badge badge-solo">ソロ盤</span>
    ...
    <div class="holo-wrap"><div class="card">
      <div class="card-cover"><img src="editions_img_4.jpg" alt="曽野舜太盤 ジャケット" loading="lazy" onerror="this.style.display='none'"></div>
      <span class="card-badge badge-solo">ソロ盤</span>
    ...
    <div class="holo-wrap"><div class="card">
      <div class="card-cover"><img src="editions_img_5.jpg" alt="山中柔太朗盤 ジャケット" loading="lazy" onerror="this.style.display='none'"></div>
      <span class="card-badge badge-solo">ソロ盤</span>
    ...
    <div class="holo-wrap"><div class="card">
      <div class="card-cover"><img src="editions_img_6.jpg" alt="吉田仁人盤 ジャケット" loading="lazy" onerror="this.style.display='none'"></div>
      <span class="card-badge badge-solo">ソロ盤</span>
    ...
    <div class="tokuten-row">
      <div class="tokuten-row-head">
        ...
        <div class="tokuten-row-body">
          ...
          <div class="tokuten-imgs">
            <img src="https://sd-milk.com/s3/skiyaki/uploads/ckeditor/pictures/860853/content_260710_M_LK_mini_album_kokuchi_04_4.jpg" alt="早期セット予約特典 クリアトレカ" loading="lazy" onclick="openAlbLb(this.src)">
          </div>
    ...
    <div class="tokuten-row">
      <div class="tokuten-row-head">
        ...
        <div class="tokuten-row-body">
          ...
          <div class="tokuten-imgs">
            <img src="https://sd-milk.com/s3/skiyaki/uploads/ckeditor/pictures/860852/content_260710_M_LK_mini_album_kokuchi_04_3.jpg" alt="早期予約特典 ペアトレカ" loading="lazy" onclick="openAlbLb(this.src)">
          </div>
          <div class="tokuten-stores">
            <span class="store-chip">VICTOR ONLINE</span><span class="store-chip">楽天ブックス</span>
            <span class="store-chip">セブンネット</span><span class="store-chip">TOWER RECORDS ONLINE</span>
...
const TOKUTEN_IMG = {
  pair:  'https://sd-milk.com/s3/skiyaki/uploads/ckeditor/pictures/860852/content_260710_M_LK_mini_album_kokuchi_04_3.jpg',
  clear: 'https://sd-milk.com/s3/skiyaki/uploads/ckeditor/pictures/860853/content_260710_M_LK_mini_album_kokuchi_04_4.jpg',
};
...
function compute() {
  ...
  if (totalItems > 0) {
    ...
    tokuten.push({
      ...
      hi:    isSelected,
      bonus: !isSelected,
      img:   isSelected ? TOKUTEN_IMG.pair : '',
    });
  }
  ...
  if (totalSets > 0) {
    ...
    tokuten.push({
      ...
      hi:    isSelected,
      bonus: !isSelected,
      img:   isSelected ? TOKUTEN_IMG.clear : '',
    });
  }
  ...
  const _tokRow = (t, bonus) => {
    const imgHtml = t.img ? `<img class="res-tok-img" src="${t.img}" alt="" loading="lazy" onerror="this.style.display='none'" onclick="openAlbLb(this.src)">` : '';
    return `<div class="res-tok${bonus ? ' bonus' : ''}">
      ${imgHtml}
      <div class="res-tok-text">
  ...
  const tokutenHtml =
    selectedToks.map(t => _tokRow(t, false)).join('') +
    (bonusToks.length ? `<div class="res-bonus-sep">購入で同時取得</div>` : '') +
    bonusToks.map(t => _tokRow(t, true)).join('');
...
<!-- Lightbox -->
<div id="alb-lb" onclick="closeAlbLb()">
  <button id="alb-lb-close" onclick="closeAlbLb()" aria-label="閉じる">&#x2715;</button>
  <img id="alb-lb-img" src="" alt="">
</div>
<script>
function openAlbLb(src){
  document.getElementById('alb-lb-img').src=src;
  document.getElementById('alb-lb').classList.add('open');
  document.body.style.overflow='hidden';
}
function closeAlbLb(){
  document.getElementById('alb-lb').classList.remove('open');
  document.getElementById('alb-lb-img').src='';
  document.body.style.overflow='';
}
document.addEventListener('keydown',function(e){if(e.key==='Escape')closeAlbLb();});
</script>
'''
...
def _render_today_schedule(events, today_str):
    """
    今日行程 HTML: スケジュール カレンダーの今日のイベントのみ表示
    """
    def _esc(s):
        return (str(s).replace('&','&amp;').replace('<','&lt;')
                      .replace('>','&gt;').replace('"','&quot;'))
    ...
    # 今日のイベント（official events only — gnews excluded, deduped by title）
    _seen_evt_titles = set()
    today_evts = []
    for e in (events or []):
        if e.get('date', '') != today_str:
            continue
        if e.get('source') == 'gnews':
            continue
        _norm_t = re.sub(r'\s+', '', e.get('title', '')).lower()
        if _norm_t in _seen_evt_titles:
            continue
        _seen_evt_titles.add(_norm_t)
        today_evts.append(e)
    if today_evts:
        for e in today_evts:
    ...
    if not today_evts:
        parts.append(
            '<p class="today-empty">本日予定されているイベントはありません。<br>'
...
def _render_live_section(events, today_str):
    """ライブ・イベント + ツアー の HTML を生成（今日含む upcoming）"""
    ...
    upcoming = sorted(
        ...
        key=lambda x: x['date'])
    # Dedup by (date, normalized-title) — same event from multiple sources
    _seen_up = set()
    _deduped_up = []
    for _e in upcoming:
        _lk = (_e.get('date',''), re.sub(r'\s+','',_e.get('title','')).lower()[:40])
        if _lk not in _seen_up:
            _seen_up.add(_lk)
            _deduped_up.append(_e)
    upcoming = _deduped_up
    if not upcoming:
        return (
            '<div class="sched-section">'
    ...
    # ── 過去のライブ記録 (collapsed) ────────────────────────────────────────
    _LIVE_KW_SET = {'ライブ', 'コンサート', '公演', 'ツアー', 'live', 'concert', 'tour',
                    'fanmeet', 'ファンミ', '[LIVE]', 'イベント', 'event'}
    past_live = sorted(
        [e for e in events
         if e.get('date', '') < today_str
         and any(kw in (e.get('title','') + e.get('details','')).lower() for kw in _LIVE_KW_SET)],
        key=lambda x: x.get('date', ''), reverse=True)
    # Dedup past live by (date, normalized-title)
    _seen_pl = set()
    _deduped_pl = []
    for _e in past_live:
        _lk = (_e.get('date',''), re.sub(r'\s+','',_e.get('title','')).lower()[:40])
        if _lk not in _seen_pl:
            _seen_pl.add(_lk)
            _deduped_pl.append(_e)
    past_live = _deduped_pl
    if past_live:
        past_rows = []
        for e in past_live:
            v   = _esc(e.get('title','').replace('[LIVE]','').strip())
            ...
            past_rows.append(
                ...
                + _brick(e['date']) +
                '<div class="ev-info">'
                '<div class="ev-badges"><span class="live-badge" style="opacity:.55">PAST</span></div>'
                f'<p class="ev-venue">{v}</p>'
                + (f'<p class="ev-city">{c}</p>' if c else '') +
                '</div></a>'
            )
        out.append(
            '<div class="sched-section">'
            '<details class="past-live-acc">'
            '<summary>&#128190; 過去のLive記録</summary>'
            '<div class="ev-list">' + ''.join(past_rows) + '</div>'
            '</details>'
            '</div>'
        )
...
def generate_schedule_html(articles, events, updated_at, official_news=None):
    ...
    prog_cards_list    = []   # 放送中 TV/ラジオ（active）
    upcoming_prog_list = []   # 出演番組 TV/ラジオ（upcoming）
    ...
    def _add_card(p_name, p_type, station, sched_d, url_use, color, dow, status='active', members=None):
        ...
        if p_type in PROG_TYPES:
            if status == 'upcoming':
                upcoming_prog_list.append(card)  # 出演番組（未放送）
            else:
                prog_cards_list.append(card)     # 放送中（active）
    ...
    if _dynamic_programs:
        for p in _dynamic_programs[:40]:
            ...
            # Skip programs that have ended
            if p.get('status') == 'ended':
                continue
            # Skip types that are never rendered (handled elsewhere or irrelevant)
            if p_type not in (*PROG_TYPES, *WORK_TYPES):
                continue
            # Dedup: only check norms of rendered types to avoid イベント/リリース poisoning
            if p_norm in seen_norms:
                continue
            if len(p_norm) >= 6 and any(
                (p_norm in s or s in p_norm) for s in seen_norms if len(s) >= 6
            ):
                continue

            color         = TYPE_COLOR.get(p_type, '#555')
            sched_display = _prog_sched_cache.get(p_name, '')
            url_use       = p.get('url', '#')
            station       = p.get('station', '')
            dow           = _prog_dow_cache.get(p_name)
            p_members     = None
            # Date-based status (already set by scraper; re-derive or correct if needed)
            p_status = p.get('status', '')
            if p_status not in ('active', 'upcoming', 'ended'):
                p_status = ''
            # Override upcoming→active when start_date has already passed
            if p_status == 'upcoming':
                _sd = p.get('start_date', '')
                if _sd and _sd <= today_jst:
                    p_status = 'active'
            # Fallback when status is still unknown
            if not p_status:
                _sd = p.get('start_date', '')
                _ed = p.get('end_date', '')
                if _ed and _ed < today_jst:
                    p_status = 'ended'
                elif _sd and _sd > today_jst:
                    p_status = 'upcoming'
                else:
                    p_status = 'active'
    ...
    # 放送中 = active TV/radio + active dramas/movies (combined into one section)
    _onair_all = prog_cards_list + work_cards_list
    prog_onair_html = (
        '<div class="prog-grid">' + ''.join(_onair_all) + '</div>'
        if _onair_all
        else (
            '<p class="today-empty">番組情報を取得できませんでした。'
            '<a href="https://sd-milk.com/contents/schedule" target="_blank" rel="noopener" '
            'style="color:var(--blue)">公式サイト</a> でご確認ください。</p>'
            if not _dynamic_programs
            else '<p class="today-empty">現在放送中の番組・作品はありません。</p>'
        )
    )
    # 出演番組 = upcoming TV/radio + upcoming dramas/movies
    _all_upcoming = upcoming_prog_list + upcoming_work_list
    prog_upcoming_html = (
        '<div class="prog-grid">' + ''.join(_all_upcoming) + '</div>'
        if _all_upcoming
        else ''
    )

    # ── Past events section ─────────────────────────────────────────────────
    _LIVE_KW = {'ライブ', 'コンサート', '公演', 'ツアー', 'live', 'concert', 'tour',
                'fanmeet', 'ファンミ', '[LIVE]', 'イベント', 'event'}
    past_events = sorted(
        [e for e in events
         if e.get('date', '') < today_jst
         and any(kw in (e.get('title', '') + e.get('details', '')).lower()
                 for kw in _LIVE_KW)
         and _milk_check(e.get('title', '') + ' ' + e.get('details', ''))],
        key=lambda x: x.get('date', ''), reverse=True
    )
    # Also include ended TV/radio/drama programs from _dynamic_programs
    _PAST_PROG_TYPES = {'TV', 'ラジオ', 'ドラマ', '映画'}
    for _pp in _dynamic_programs:
        if _pp.get('status') != 'ended':
            continue
        if _pp.get('type', '') not in _PAST_PROG_TYPES:
            continue
        _pp_date = _pp.get('end_date', '') or _pp.get('start_date', '')
        if not _pp_date or _pp_date >= today_jst:
            continue
        past_events.append({
            'date':    _pp_date,
            'title':   _pp.get('name', ''),
            'details': _pp.get('station', '') or _pp.get('type', ''),
            'url':     _pp.get('url', '#'),
        })
    past_events.sort(key=lambda x: x.get('date', ''), reverse=True)
    past_html = ''
    if past_events:
        _cur_year = str(datetime.now(JST).year)
        by_year = {}
        for e in past_events:
            yr = e.get('date', '')[:4]
            by_year.setdefault(yr, []).append(e)

        def _past_rows(event_list):
            rows = []
            for e in event_list:
                mm_dd  = e.get('date', '')[5:]
        ...
        for yr in sorted(by_year.keys(), reverse=True):
            rows = _past_rows(by_year[yr])
            if yr == _cur_year:
                # Current year — always expanded
                year_blocks.append(
                ...
                # Previous years — collapsed accordion
                year_blocks.append(
                    f'<details class="past-acc">'
                    f'<summary>'
                    f'<span class="past-acc-year">{esc(yr)} 年</span>'
                    f'<span class="past-acc-count">{len(rows)} 件</span>'
                    f'</summary>'
                    f'<div class="past-list">{"".join(rows)}</div>'
                    f'</details>'
                )
    ...
    # ── Schedule calendar data (official events only — no gnews noise) ──
    import json as _json
    from datetime import timedelta as _td
    cal_events = [e for e in events if e.get('source') != 'gnews']
    ...
    # Add DOW-based recurring calendar entries for dynamic programs (TV/radio with known airing day)
    for _dp in _dynamic_programs:
        if _dp.get('status') != 'active':
            continue
        if _dp.get('type') not in ('TV', 'ラジオ'):
            continue
        _dp_name = _dp.get('name', '')
        _dp_dow = _prog_dow_cache.get(_dp_name)
        if not _dp_dow:
            continue
        _dp_title = _dp_name
        for _dw in _dp_dow:
            _days_back = (_today_dt.weekday() - _dw) % 7
            _base = _today_dt - _td(days=_days_back)
            for _w in range(-4, 9):
                _cand = _base + _td(weeks=_w)
                _ds = _cand.strftime('%Y-%m-%d')
                if not any(e.get('date') == _ds and e.get('title') == _dp_title for e in cal_events):
                    cal_events.append({'date': _ds, 'title': _dp_title,
                                       'details': _dp.get('station', ''), 'url': _dp.get('url', '#')})
    ...
    today_sched   = _render_today_schedule(events, today_jst)

    # 出演番組 section = upcoming TV/radio + upcoming works (always shown)
    upcoming_section = (
        '<div class="sched-section">'
        '<p class="sched-section-title">&#128225; 出演番組・公開予定</p>'
        + (prog_upcoming_html if prog_upcoming_html
           else '<p class="today-empty">現在、公開予定の新番組・出演情報はありません。</p>')
        + '</div>'
    )

    # ── Official news categorized section ──────────────────────────────────
    _CAT_COLOR = {
        'LIVE': '#00695c', 'MEDIA': '#1565c0', 'RELEASE': '#6a1b9a',
        'TICKET': '#bf360c', 'GOODS': '#e65100', 'FC': '#37474f', 'OTHER': '#546e7a',
    }
    _CAT_LABEL = {
        'LIVE': 'LIVE', 'MEDIA': 'MEDIA', 'RELEASE': 'RELEASE',
        'TICKET': 'TICKET', 'GOODS': 'GOODS', 'FC': 'FC', 'OTHER': 'OTHER',
    }
    if official_news:
        _all_cats = ['ALL', 'LIVE', 'MEDIA', 'RELEASE', 'TICKET', 'GOODS', 'FC']
        _tab_html = ''.join(
            f'<button class="offnews-tab{" active" if c == "ALL" else ""}" '
            f'onclick="offnewsFilter(this,\'{c}\')">{c}</button>'
            for c in _all_cats
        )
        _rows = []
        for item in sorted(official_news, key=lambda x: x.get('date', ''), reverse=True):
            cat  = item.get('category', 'OTHER')
            col  = _CAT_COLOR.get(cat, '#546e7a')
            lbl  = _CAT_LABEL.get(cat, cat)
            date = item.get('date', '')
            title = esc(item.get('title', ''))
            url   = esc(item.get('url', '#'))
            _rows.append(
                f'<a class="offnews-item" href="{url}" target="_blank" rel="noopener" data-cat="{cat}">'
                f'<span class="offnews-cat" style="background:{col}">{lbl}</span>'
                f'<span class="offnews-date">{date}</span>'
                f'<span class="offnews-title">{title}</span>'
                f'</a>'
            )
        _filter_js = (
            '<script>'
            'function offnewsFilter(btn,cat){'
            'document.querySelectorAll(".offnews-tab").forEach(b=>b.classList.remove("active"));'
            'btn.classList.add("active");'
            'document.querySelectorAll(".offnews-item").forEach(el=>{'
            'el.style.display=(cat==="ALL"||el.dataset.cat===cat)?"":"none";'
            '});}'
            '</script>'
        )
        official_news_section = (
            '<div class="sched-section">'
            '<p class="sched-section-title">&#128240; 公式ニュース（sd-milk.com）</p>'
            '<div class="offnews-tabs">' + _tab_html + '</div>'
            '<div class="offnews-list">' + ''.join(_rows) + '</div>'
            + _filter_js +
            '</div>'
        )
    else:
    ...
    return (SCHEDULE_TEMPLATE
            ...
            .replace('__PROG_ONAIR__', prog_onair_html)
            .replace('__PROG_UPCOMING_SECTION__', upcoming_section)
            .replace('__OFFICIAL_NEWS_SECTION__', official_news_section)
            .replace('__PAST_EVENTS__', past_html)
            .replace('__SCHED_DATA__', sched_data_js)
            .replace('__LIVE_SECTION__', live_section))

# ── Editions cover image downloader ──────────────────────────────────────────
# sd-milk.com requires Referer: https://sd-milk.com to serve these images.
# We download them once to the repo so the fan site can serve them directly.
_EDITIONS_IMAGES = [
    ('editions_img_0.jpg', 'https://sd-milk.com/s3/skiyaki/uploads/ckeditor/pictures/860904/content_%E9%80%9A%E5%B8%B8%E7%9B%A4_0709_MILK_LE_JKT_VICL-66166%E3%81%AE%E3%82%B3%E3%83%94%E3%83%BC.jpg'),
    ('editions_img_1.jpg', 'https://sd-milk.com/s3/skiyaki/uploads/ckeditor/pictures/860903/content_%E5%88%9D%E5%9B%9E%E7%9B%A4_0709_MILK_LE_JKT_VIZL2547%E3%81%AE%E3%82%B3%E3%83%94%E3%83%BC.jpg'),
    ('editions_img_2.jpg', 'https://sd-milk.com/s3/skiyaki/uploads/ckeditor/pictures/860902/content_%E4%BD%90%E9%87%8E%E7%9B%A4_0709_MILK_LE_JKT_VICL-66177%E3%81%AE%E3%82%B3%E3%83%94%E3%83%BC.jpg'),
    ('editions_img_3.jpg', 'https://sd-milk.com/s3/skiyaki/uploads/ckeditor/pictures/860901/content_%E5%A1%A9%EF%A8%91%E7%9B%A4_0709_MILK_LE_JKT_VICL-66178%E3%81%AE%E3%82%B3%E3%83%94%E3%83%BC.jpg'),
    ('editions_img_4.jpg', 'https://sd-milk.com/s3/skiyaki/uploads/ckeditor/pictures/860899/content_%E6%9B%BD%E9%87%8E%E7%9B%A4_0709_MILK_LE_JKT_VICL-66179%E3%81%AE%E3%82%B3%E3%83%94%E3%83%BC.jpg'),
    ('editions_img_5.jpg', 'https://sd-milk.com/s3/skiyaki/uploads/ckeditor/pictures/860900/content_%E5%B1%B1%E4%B8%AD%E7%9B%A4_0709_MILK_LE_JKT_VICL-66180%E3%81%AE%E3%82%B3%E3%83%94%E3%83%BC.jpg'),
    ('editions_img_6.jpg', 'https://sd-milk.com/s3/skiyaki/uploads/ckeditor/pictures/860898/content_%E5%90%89%E7%94%B0%E7%9B%A4_0709_MILK_LE_JKT_VICL-66181%E3%81%AE%E3%82%B3%E3%83%94%E3%83%BC.jpg'),
]

def _download_editions_images():
    dl_headers = {**HEADERS, 'Referer': 'https://sd-milk.com/contents/news/release'}
    for fname, url in _EDITIONS_IMAGES:
        ...
        try:
            ...
            if r.status_code == 200 and r.content:
                with open(fname, 'wb') as f:
                    f.write(r.content)
                print(f'  Downloaded editions image: {fname}')
            else:
                print(f'  editions image download failed ({r.status_code}): {fname}')
        except Exception as e:
            print(f'  editions image download error {fname}: {e}')
...
def main():
    ...
    # Pattern handles: " - Oricon News", " - Billboard Japan", "｜Oricon News" etc.
    _fixed_titles = 0
    _gnews_suffix_re = re.compile(
        r'(?:'
        r'\s+[-–—]\s+\S+(?:\s+\S+)?\s*'   # " - Oricon News" or " - Oricon"
        r'|'
        r'\s*[|｜]\s*\S+(?:\s+\S+)?\s*'   # "｜Oricon News" or "| BARKS"
        r')$'
    )
    for _a in existing:
        if 'news.google.com' not in _a.get('url', ''):
            # Also clean non-gnews Oricon articles that have source suffix in summary
            if 'oricon.co.jp' in _a.get('url', ''):
                for _fld in ('summary_ja', 'title_ja'):
                    _v = _a.get(_fld, '')
                    _c = _gnews_suffix_re.sub('', _v).strip()
                    if _c != _v:
                        _a[_fld] = _c
                        _a[_fld.replace('_ja', '_zh')] = ''
            continue
        tj = _a.get('title_ja', '')
    ...
    # Fix existing Oricon photo-split articles: normalize URL to /full/ and recompute id
    # Also back-fill images from photo sub-pages for articles that only have 1 image.
    _oricon_repaired = 0
    for _a in existing:
        _raw_url = _a.get('url', '')
        if 'oricon.co.jp' not in _raw_url:
            continue
        _canonical = _norm_oricon_url(_raw_url)
        if _canonical != _raw_url:
            _a['url'] = _canonical
            _a['id']  = make_id(_canonical)
        # Back-fill photo sub-pages if images list is short (≤1)
        _cur_imgs = _a.get('images', [])
        if len(_cur_imgs) <= 1:
            _base = re.sub(r'/full/?$', '', _canonical.rstrip('/'))
            _seen_ph = set(_cur_imgs)
            _new_imgs = list(_cur_imgs)
            for _i in range(1, 13):
                _photo_url = f'{_base}/photo/{_i}/'
                try:
                    _pr = requests.get(_photo_url, headers=HEADERS, timeout=8)
                    if _pr.status_code != 200:
                        break
                    ...
                    if _og:
                        _ph = _og.get('content', '').strip()
                        if _ph and _ph not in _seen_ph:
                            _seen_ph.add(_ph)
                            _new_imgs.append(_ph)
                    time.sleep(0.2)
                except Exception:
                    break
            if len(_new_imgs) > len(_cur_imgs):
                _a['images'] = _new_imgs
                if not _a.get('image') and _new_imgs:
                    _a['image'] = _new_imgs[0]
                _oricon_repaired += 1
    if _oricon_repaired:
        print(f'  Back-filled photo gallery images for {_oricon_repaired} Oricon articles')

    # After URL repair, dedup existing by URL — repair may have made multiple entries identical.
    # Sort content-rich entries first so dedup_by_url retains the best version (not /comment/ etc.)
    _before_url_dedup = len(existing)
    existing.sort(key=lambda a: (len(a.get('body_ja') or ''), len(a.get('images') or [])), reverse=True)
    existing = dedup_by_url(existing)
    if len(existing) < _before_url_dedup:
        print(f'  URL dedup after repair collapsed {_before_url_dedup - len(existing)} duplicate entries')

    # Photo gallery consolidation: merge images into parent article then remove gallery cards
    _photo_in_archive = [
        _a for _a in existing
        if _is_photo_page(_a.get('url',''), _a.get('title_ja','') or _a.get('title',''))
    ]
    if _photo_in_archive:
        _url_to_art = {_a.get('url',''): _a for _a in existing if _a.get('url')}
        _merged_count = 0
        for _pp in _photo_in_archive:
            try:
                _r = requests.get(_pp['url'], headers=HEADERS, timeout=8)
                _soup = BeautifulSoup(_r.text, 'html.parser')
                # Find canonical / og:url → parent article URL
                _parent_url = ''
                for _attrs in [{'property': 'og:url'}, {'rel': 'canonical'}]:
                    _el = _soup.find('meta', _attrs) or _soup.find('link', _attrs)
                    if _el:
                        _cand = _el.get('content') or _el.get('href', '')
                        if _cand and not _is_photo_page(_cand):
                            _parent_url = _cand
                            break
                _parent = _url_to_art.get(_parent_url)
                if _parent:
                    _seen_imgs = set(_parent.get('images', []))
                    for _itag in _soup.find_all('img', src=True):
                        _src = _clean_img_url(_itag.get('src','').strip())
                        if (_src.startswith('http') and _src not in _seen_imgs
                                and not any(w in _src.lower() for w in _SKIP_IMG_WORDS)):
                            _seen_imgs.add(_src)
                            _parent.setdefault('images', []).append(_src)
                    _merged_count += 1
            except Exception:
                pass
            time.sleep(0.3)
        # Remove all photo gallery pages from existing
        _before_photo_all = len(existing)
        existing = [
            _a for _a in existing
            if not _is_photo_page(_a.get('url',''), _a.get('title_ja','') or _a.get('title',''))
        ]
        print(f'  Photo gallery cleanup: merged {_merged_count}, removed {_before_photo_all - len(existing)} cards')

    # Extra cleanup: Google News URL articles with short/no body are likely photo sub-pages
    # (Modelpress /photo/detail/, Oricon /photo/N/, Sanspo /photo/XXX/, etc.) that slipped
    # through because GNews URLs are opaque to _is_photo_page().
    _gn_candidates = [
        _a for _a in existing
        if 'news.google.com' in _a.get('url', '')
        and len(_a.get('body_ja', '') or '') < 400  # photo/gallery pages have minimal body
    ]
    if _gn_candidates:
        _gn_remove = set()
        for _gnart in _gn_candidates[:40]:
            try:
                _real = _resolve_gnews_url(_gnart['url'])
                if not _real:
                    continue
                if _is_photo_page(_real):
                    _gn_remove.add(_gnart['url'])
                elif 'oricon.co.jp' in _real:
                    # Oricon photo sub-pages get stripped to main URL by _norm_oricon_url.
                    # If normalized form differs, it was a photo sub-page.
                    _norm = _norm_oricon_url(_real)
                    _real_clean = _real.rstrip('/') + '/'
                    if _norm and _norm != _real_clean:
                        _gn_remove.add(_gnart['url'])
            except Exception:
                pass
        if _gn_remove:
            _before_gn = len(existing)
            existing = [_a for _a in existing if _a.get('url', '') not in _gn_remove]
            print(f'  GNews photo cleanup: removed {_before_gn - len(existing)} photo pages')

    # Repair pass: re-fetch body for articles missing body_ja OR with low-quality body
    # (short body, or body that looks like it mostly contains link text / noise)
    _body_repaired = 0
    _body_cutoff   = (datetime.now(JST) - timedelta(days=90)).strftime('%Y-%m-%d')

    def _body_needs_repair(art):
        bj = art.get('body_ja') or ''
        if not bj:
            return True
        if len(bj) < 120:  # suspiciously short — likely failed extraction
            return True
        # Body with lots of URLs likely contains unfiltered related-link text
        url_count = len(re.findall(r'https?://', bj))
        if url_count >= 3:
            return True
        return False

    for _a in existing:
        if _body_repaired >= 60:
            break
        if not _a.get('url'):
            continue
        if not _body_needs_repair(_a):
            continue
        if (_a.get('date', '') or '')[:10] < _body_cutoff:
            continue
        _img, _, _body, _, _ = fetch_article_data(_a['url'])
        if _body:
            _a['body_ja'] = _body
            _body_repaired += 1
        if _img and not _a.get('image'):
            _a['image'] = _img
        time.sleep(0.3)
    if _body_repaired:
        print(f'  Back-filled body_ja for {_body_repaired} articles')

    # Image repair pass: re-fetch image for recent articles that have body_ja but no image
    _img_repaired = 0
    _img_cutoff = (datetime.now(JST) - timedelta(days=90)).strftime('%Y-%m-%d')
    for _a in existing:
        if _img_repaired >= 30:
            break
        if _a.get('image') or not _a.get('url'):
            continue
        if (_a.get('date', '') or '')[:10] < _img_cutoff:
            continue
        _img, _, _, _, _ = fetch_article_data(_a['url'])
        if _img:
            _a['image'] = _img
            _img_repaired += 1
        time.sleep(0.3)
    if _img_repaired:
        print(f'  Back-filled image for {_img_repaired} articles')

    # Dedup existing archive by URL then by title (removes cross-source duplicates)
    existing = dedup_by_url(existing)
    existing = dedup_by_title(existing)

    # Back-fill missing translations — each pass has its own cap so body pass always runs
    if translator or _google_translate_ok:
        _tx_title = _tx_summary = _tx_body = 0
        # Pass 1: missing title translations (cap 200)
        for _a in existing:
            if _tx_title >= 200:
                break
            if _a.get('title_ja') and not _a.get('title_zh'):
                ...
                if result:
                    _a['title_zh'] = result
                    _tx_title += 1
        # Pass 2: missing summary translations (cap 200)
        for _a in existing:
            if _tx_summary >= 200:
                break
            if _a.get('summary_ja') and not _a.get('summary_zh'):
                ...
                if result:
                    ...
                    _tx_summary += 1
        # Pass 3: missing body translations — this is what users see in modal (cap 500)
        for _a in existing:
            if _tx_body >= 500:
                break
            if _a.get('body_ja') and not _a.get('body_zh'):
                ...
                if result:
                    _a['body_zh'] = result
                    _tx_body += 1
        _tx_total = _tx_title + _tx_summary + _tx_body
        if _tx_total:
            print(f'Back-filled translations: title={_tx_title} summary={_tx_summary} body={_tx_body}')
    ...
    # Merge images from new photo gallery pages into their parent articles before filtering
    _new_photos = [a for a in new_raw
                   if _is_photo_page(a.get('url',''), a.get('title_ja','') or a.get('title',''))]
    _new_mains  = [a for a in new_raw
                   if not _is_photo_page(a.get('url',''), a.get('title_ja','') or a.get('title',''))]
    if _new_photos:
        # Build lookup: (source, date) → main article, for quick parent matching
        _main_by_src_date = {}
        for _m in _new_mains:
            _k = (_m.get('source',''), (_m.get('date','') or '')[:10])
            _main_by_src_date.setdefault(_k, []).append(_m)
        # Also build lookup against existing archive
        _exist_by_url = {_a.get('url',''): _a for _a in existing if _a.get('url')}

        for _pp in _new_photos:
            try:
                _pr = requests.get(_pp['url'], headers=HEADERS, timeout=8)
                _ps = BeautifulSoup(_pr.text, 'html.parser')
                # Find parent via og:url / canonical
                _parent = None
                for _at in [{'property': 'og:url'}, {'rel': 'canonical'}]:
                    _el = _ps.find('meta', _at) or _ps.find('link', _at)
                    if _el:
                        _pu = _el.get('content') or _el.get('href','')
                        if _pu and not _is_photo_page(_pu):
                            _parent = _exist_by_url.get(_pu)
                            if not _parent:
                                # Check new mains by URL
                                for _m in _new_mains:
                                    if _m.get('url','') == _pu:
                                        _parent = _m
                                        break
                            break
                # Fallback: match by source + date if og:url not found
                if not _parent:
                    _k = (_pp.get('source',''), (_pp.get('date','') or '')[:10])
                    _candidates = _main_by_src_date.get(_k, [])
                    if len(_candidates) == 1:
                        _parent = _candidates[0]
                # Merge images into parent
                if _parent:
                    _seen_i = set(_parent.get('images', []))
                    for _itag in _ps.find_all('img', src=True):
                        _src = _clean_img_url(_itag.get('src','').strip())
                        if (_src.startswith('http') and _src not in _seen_i
                                and not any(w in _src.lower() for w in _SKIP_IMG_WORDS)):
                            _seen_i.add(_src)
                            _parent.setdefault('images', []).append(_src)
            except Exception:
    ...
    # Remove photo gallery pages — their images have been merged above
    new_raw = _new_mains
    ...
    for a in new_raw:
        if a.get('url'):
            img, imgs, body, pub_date, real_url = fetch_article_data(a['url'])
            # Update URL to the resolved real URL (important for Google News redirect URLs
            # so _is_photo_page() can correctly detect sanspo/oricon photo sub-pages)
            if real_url and real_url != a['url']:
                a['url'] = real_url
            if img:                        # never overwrite a good image with empty
                a['image'] = img
            if imgs:
                a['images'] = imgs
            ...
            if body:
                a['body_ja'] = body
            time.sleep(0.3)
    # Re-filter photo pages now that Google News URLs have been resolved to real URLs
    _before_re = len(new_raw)
    new_raw = [a for a in new_raw
               if not _is_photo_page(a.get('url', ''), a.get('title_ja', '') or a.get('title', ''))]
    if len(new_raw) < _before_re:
        print(f'  Post-resolve photo filter removed {_before_re - len(new_raw)} photo pages')
    ...
    # Remove photo sub-page articles (existing entries that slipped through in earlier runs)
    before_ph = len(merged)
    merged = [a for a in merged
              if not _is_photo_page(a.get('url', ''),
                                    a.get('title_ja', '') or a.get('title', ''))]
    if len(merged) < before_ph:
        print(f'  Photo-page cleanup removed {before_ph - len(merged)} entries')
    print(f'News: +{added} new (total {len(merged)})')
    save_json(ARTICLES_FILE, merged)

    # Round displayed timestamp to the nearest scheduled update slot
    # so the page always shows 09:00 / 12:00 / 18:00 / 00:00 JST regardless of
    # Snap to nearest scheduled hour only if within 30 min (hides cron jitter).
    # Manual triggers that run far from any schedule show the actual time.
    _SCHEDULED_HOURS = [0, 9, 12, 18]
    _now = datetime.now(JST)
    _h, _m = _now.hour, _now.minute
    _nearest = min(_SCHEDULED_HOURS, key=lambda s: min((_h - s) % 24, (s - _h) % 24))
    _diff_min = min(abs(_h * 60 + _m - _nearest * 60),
                    24 * 60 - abs(_h * 60 + _m - _nearest * 60))
    if _diff_min <= 30:
        now_jst = _now.replace(hour=_nearest, minute=0, second=0, microsecond=0).strftime('%Y-%m-%d %H:%M')
    else:
        now_jst = _now.strftime('%Y-%m-%d %H:%M')
    ...
    official_news = fetch_sd_milk_official_news()
    new_events    = fetch_schedule(official_news=official_news)
    gnews_events  = fetch_gnews_schedule_events()
    # Merge Google News events in; prefer existing entries (same id) over gnews duplicates
    gnews_new = [e for e in gnews_events if e['id'] not in {x['id'] for x in new_events}]
    new_events = new_events + gnews_new
    # For source='official' events: allow date updates (titles rarely contain event dates —
    # the correct date is only found by fetching the article body on each run).
    new_by_id = {e['id']: e for e in new_events if e.get('id')}
    updated_sched = 0
    for ev in ex_schedule:
        if ev.get('source') == 'official' and ev.get('id') in new_by_id:
            fresh = new_by_id[ev['id']]
            if fresh.get('date') and fresh['date'] != ev.get('date'):
                ev['date'] = fresh['date']
                updated_sched += 1
    merged_sched, added_sched = merge_by_id(ex_schedule, new_events)
    print(f'Schedule: +{added_sched} new, {updated_sched} date-corrected (total {len(merged_sched)})')
    ...
    for g in new_goods:
        if g.get('url') and not g.get('image'):
            img, _, _, _, _ = fetch_article_data(g['url'])
            if img:
                g['image'] = img
            time.sleep(0.2)
    ...
    # ── Release/Goods official announcements (sd-milk /news/release + /news/goods) ──
    RELEASES_FILE = 'releases.json'
    ex_releases  = load_json(RELEASES_FILE)
    ex_rel_ids   = {r['id'] for r in ex_releases}
    new_releases = fetch_sd_milk_release_news(ex_ids=ex_rel_ids)
    # Translate new release titles
    if translator or _google_translate_ok:
        for rv in new_releases:
            if rv.get('title') and not rv.get('title_zh'):
                rv['title_zh'] = translate(rv['title'])
    # Merge: new releases + existing (dedup by id), keep latest 60
    new_rel_ids   = {r['id'] for r in new_releases}
    kept_releases = [r for r in ex_releases if r['id'] not in new_rel_ids]
    merged_releases = sorted(
        new_releases + kept_releases,
        key=lambda x: x.get('date', ''), reverse=True
    )[:60]
    save_json(RELEASES_FILE, merged_releases)
    print(f'Releases: {len(merged_releases)} total ({len(new_releases)} new)')

    global _dynamic_programs
    _dynamic_programs = fetch_sd_milk_programs_pw()   # auto-fetch program list first
    fetch_prog_schedules()                             # then populate schedule/DOW cache from dynamic URLs
    with open('schedule.html', 'w', encoding='utf-8') as f:
        f.write(generate_schedule_html(merged, merged_sched, now_jst, official_news=official_news))

    with open('goods.html', 'w', encoding='utf-8') as f:
        f.write(generate_goods_html(merged_goods, now_jst, releases=merged_releases))

    with open('album.html', 'w', encoding='utf-8') as f:
        f.write(generate_album_html(now_jst))

    # ── Oricon Charts (weekly) ────────────────────────────────────────────────
    ex_charts        = load_json(CHARTS_FILE)
    new_charts, newly_scanned = fetch_oricon_charts()
    ex_chart_ids     = {c['id'] for c in ex_charts if c.get('id')}
    added_charts     = [c for c in new_charts if c['id'] not in ex_chart_ids]
    merged_charts    = new_charts + [c for c in ex_charts if c.get('id') and c['id'] not in {e['id'] for e in new_charts}]
    merged_charts    = sorted(merged_charts, key=lambda c: c['date'], reverse=True)
    if newly_scanned:
        ex_scanned = load_json(CHART_SCANNED_FILE)
        ex_scanned_set = {(e['chart_code'], e['date']) for e in ex_scanned}
        combined_scanned = ex_scanned + [s for s in newly_scanned
                                          if (s['chart_code'], s['date']) not in ex_scanned_set]
        combined_scanned = sorted(combined_scanned, key=lambda s: s['date'], reverse=True)[:6*52]
        save_json(CHART_SCANNED_FILE, combined_scanned)

    # ── Oricon Charts (daily) ─────────────────────────────────────────────────
    new_daily, newly_daily_scanned = fetch_oricon_daily_charts()
    ex_daily_ids = {c['id'] for c in merged_charts if c.get('period_type') == 'daily'}
    added_daily  = [c for c in new_daily if c['id'] not in ex_daily_ids]
    if added_daily:
        daily_cutoff = (datetime.now(JST).replace(tzinfo=None) - timedelta(days=65)).strftime('%Y-%m-%d')
        # Drop daily entries older than 45 days, then prepend new ones
        merged_charts = [c for c in merged_charts
                         if c.get('period_type') != 'daily' or c['date'] >= daily_cutoff]
        merged_charts = added_daily + merged_charts
    if newly_daily_scanned:
        ex_daily_scanned = load_json(CHART_DAILY_SCANNED_FILE)
        ex_daily_set = {(e['chart_code'], e['date']) for e in ex_daily_scanned}
        combined_daily = ex_daily_scanned + [s for s in newly_daily_scanned
                                              if (s['chart_code'], s['date']) not in ex_daily_set]
        combined_daily = sorted(combined_daily, key=lambda s: s['date'], reverse=True)[:4*45]
        save_json(CHART_DAILY_SCANNED_FILE, combined_daily)

    # ── Billboard Japan Charts ────────────────────────────────────────────────
    new_bj, newly_bj_scanned = fetch_billboard_japan_charts()
    print(f'Fetching Billboard Japan charts...')
    ex_merged_ids = {c['id'] for c in merged_charts if c.get('id')}
    added_bj = [c for c in new_bj if c['id'] not in ex_merged_ids]
    if added_bj:
        merged_charts = added_bj + merged_charts
    if newly_bj_scanned:
        ex_bj_sc  = load_json(CHART_BJ_SCANNED_FILE)
        ex_bj_set = {(e['chart_code'], e['date']) for e in ex_bj_sc}
        combined_bj = ex_bj_sc + [s for s in newly_bj_scanned
                                    if (s['chart_code'], s['date']) not in ex_bj_set]
        save_json(CHART_BJ_SCANNED_FILE, combined_bj)

    save_json(CHARTS_FILE, merged_charts)
    print(f'Charts: +{len(added_charts)} Oricon weekly, +{len(added_daily)} Oricon daily, +{len(added_bj)} Billboard Japan (total {len(merged_charts)})')
    with open('chart.html', 'w', encoding='utf-8') as f:
        f.write(generate_chart_html(merged_charts, now_jst, source='oricon'))
    with open('chart_bj.html', 'w', encoding='utf-8') as f:
        f.write(generate_chart_html(merged_charts, now_jst, source='bj'))
