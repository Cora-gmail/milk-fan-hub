...
def generate_goods_html(goods, updated_at):
    def esc(s):
...
def main():
    ...
    # Remove Oricon photo gallery articles from existing archive (they're duplicates)
    _photo_title_re = re.compile(r'^(画像|写真)[・・]')
    _before_photo = len(existing)
    existing = [_a for _a in existing if not (
        'oricon.co.jp' in (_a.get('url', '')) and
        bool(_photo_title_re.match(_a.get('title_ja', '') or _a.get('title', '') or ''))
    )]
    if len(existing) < _before_photo:
        print(f'  Removed {_before_photo - len(existing)} Oricon photo gallery articles from archive')

    # Repair pass: re-fetch body for recent articles missing body_ja (new selectors catch more sites)
    _body_repaired = 0
    _body_cutoff   = (datetime.now(JST) - timedelta(days=45)).strftime('%Y-%m-%d')
    for _a in existing:
        if _body_repaired >= 25:
            break
        if _a.get('body_ja') or not _a.get('url'):
            continue
        if (_a.get('date', '') or '')[:10] < _body_cutoff:
            continue
        _, _, _body, _ = fetch_article_data(_a['url'])
        if _body:
            _a['body_ja'] = _body
            _body_repaired += 1
            time.sleep(0.3)
    ...
    with open('goods.html', 'w', encoding='utf-8') as f:
        f.write(generate_goods_html(merged_goods, now_jst))
