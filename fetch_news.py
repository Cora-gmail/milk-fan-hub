...
_ARTICLE_BODY_SELECTORS = [
    ...
    '.block-detail-body .mod-p',                     # Oricon News（実測確認済み）
    '.block-detail-body',                            # Oricon News fallback
...
def _goods_card_html(g, esc):
    ...
    url      = esc(raw_url) if has_url else ''
    img      = g.get('image', '')
    img_block = (
    ...
    inner = (
        ...
        + (f'<span class="goods-buy">詳細・購入 &rarr;</span>' if has_url else '')
        + '</div>'
    )
    if has_url:
        return f'<a class="goods-card" href="{url}" target="_blank" rel="noopener">{img_block}{inner}</a>'
    return f'<div class="goods-card">{img_block}{inner}</div>'
...
def generate_schedule_html(articles, events, updated_at, official_news=None):
    ...
    if _dynamic_programs:
        for p in _dynamic_programs[:40]:
            ...
            p_members     = None
            # Date-based status (already set by scraper; re-derive if missing)
            p_status = p.get('status', '')
            if not p_status or p_status not in ('active', 'upcoming', 'ended'):
                sd = p.get('start_date', '')
