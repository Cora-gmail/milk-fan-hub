...
def fetch_schedule():
    ...
    # ── Step 1: Try __NEXT_DATA__ (Next.js/React SPA pattern) ────────────────
    next_urls = [
...
<style>
__CSS__
.schedule-wrap{max-width:1100px;margin:24px auto;padding:0 16px 60px;}
.schedule-links{display:flex;gap:10px;flex-wrap:wrap;margin-bottom:18px;}
.schedule-link-btn{display:inline-flex;align-items:center;gap:6px;padding:9px 20px;border-radius:22px;font-size:.83rem;font-weight:600;text-decoration:none;transition:opacity .2s;}
.schedule-link-btn:hover{opacity:.82;}
.schedule-link-btn.primary{background:var(--blue);color:#fff;}
.schedule-link-btn.secondary{background:var(--blue3);color:var(--blue);border:1px solid var(--blue);}
.embed-frame{width:100%;min-height:82vh;border:none;border-radius:14px;box-shadow:var(--shadow);display:block;background:#fff;}
.embed-note{margin-top:10px;font-size:.72rem;color:var(--muted);text-align:center;}
</style>
</head>
<body>
<header class="site-header">
...
<div class="schedule-wrap">
  <div class="schedule-links">
    <a class="schedule-link-btn primary" href="https://sd-milk.com/calendar"
       target="_blank" rel="noopener">&#128197; 公式カレンダー &#8599;</a>
    <a class="schedule-link-btn secondary" href="https://sd-milk.com/contents/schedule"
       target="_blank" rel="noopener">&#128203; スケジュール一覧 &#8599;</a>
  </div>
  <iframe
    class="embed-frame"
    src="https://sd-milk.com/calendar"
    title="M!LK 公式カレンダー"
    loading="lazy"
    sandbox="allow-scripts allow-same-origin allow-popups allow-forms">
  </iframe>
  <p class="embed-note">
    内容が表示されない場合は上のリンクから公式サイトをご確認ください。
    ／ 若無法顯示，請點上方連結前往官網確認。
  </p>
</div>
<footer>
  <p>m!lk Fan Hub &nbsp;&middot;&nbsp; 演出情報は sd-milk.com 公式サイトより &nbsp;&middot;&nbsp; 毎日 09:00 JST 自動更新</p>
  __SOURCES_FOOTER__
</footer>
</body>
</html>'''
...
def generate_schedule_html(updated_at):
    css = COMMON_CSS
    sc_json = json.dumps(SOURCE_COLORS, ensure_ascii=False)
...
def main():
    ...
    with open('schedule.html', 'w', encoding='utf-8') as f:
        f.write(generate_schedule_html(now_jst))
