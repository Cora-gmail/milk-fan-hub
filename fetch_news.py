...
def _parse_oricon_daily_chart_page(chart_code, chart_name, date_str):
    """Fetch Oricon DAILY chart page; return entries matching M!LK/members."""
...
def fetch_oricon_charts():
    ...
    today = datetime.now(JST).replace(tzinfo=None)
    # Oricon weekly chart dates are Sundays — use last Sunday as base
    days_since_sunday = (today.weekday() + 1) % 7  # 0 if today is Sun, 1 if Mon … 6 if Sat
    last_sunday = today - timedelta(days=days_since_sunday)
    this_week_str = last_sunday.strftime('%Y-%m-%d')
    ...
    chart_dates = [(last_sunday - timedelta(weeks=w)).strftime('%Y-%m-%d') for w in range(12)]
...
def _norm_oricon_url(url):
    ...
    if 'oricon.co.jp' not in url:
        return url
    url = url.rstrip('/')
    url = re.sub(r'/(?:embed(?:/video)?|video)', '', url)   # strip embed/video anywhere
...
def fetch_schedule(official_news=None):
    ...
    try:
        ...
        for item in _off_news:
            ...
            date_str = _extract_event_date_from_text(title_ja, '', item.get('date', ''))
            events.append({
                'id':       item['id'],
                'date':     date_str,
                'title':    title_ja,
            ...
            if nd:
                ...
                for a in nd:
                    ...
                    if any(kw in combined for kw in SCHED_KW) and a.get('date'):
                        ...
                        # Use actual event date extracted from text when available
                        date_str = _extract_event_date_from_text(title_ja, summary_ja, pub_date)
                        href = a.get('url', '')
                        news_arts.append({
...
def _render_today_schedule(events, today_dow, today_str):
    ...
    # ── ① 今日放送のレギュラー番組（_dynamic_programs + _prog_dow_cache）──────────
    TYPE_COLOR = {'TV': '#b71c1c', 'ラジオ': '#0277bd'}
    airing = []
    for p in _dynamic_programs:
        if p.get('status', 'upcoming') != 'active':
            continue
        p_type = p.get('type', '')
        if p_type not in ('TV', 'ラジオ'):
            continue
        name   = p.get('name', '')
        dow    = _prog_dow_cache.get(name)
        # Skip only when DOW is known AND today doesn't match; show if DOW unknown
        if dow is not None and today_dow not in dow:
            continue
        sched_display = _prog_sched_cache.get(name, p.get('station', ''))
        color = TYPE_COLOR.get(p_type, '#555')
        airing.append((name, p_type, p.get('station', ''), sched_display, p.get('url', '#'), color))
...
def main():
    ...
    print(f'Charts: +{len(added_charts)} weekly, +{len(added_daily)} daily (total {len(merged_charts)})')
